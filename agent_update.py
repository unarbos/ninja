...
Key behavior change vs. prior challenger:
- Adds a "best patch so far" checkpoint that is captured throughout the loop
  whenever a substantive non-trivial patch is on disk.
- Adds a bounded second attempt when the first attempt produces an empty or
  low-signal patch and meaningful wall-clock time remains, while keeping the
  first-attempt patch as a fallback if the retry regresses.
- Tightens wall-clock budgeting so we always lock in a partial patch before
  the validator's per-task timeout fires.
"""
...
WALL_CLOCK_FIRST_ATTEMPT_SAFE = 130.0
RETRY_MIN_HEADROOM_SECONDS = 100.0
LOW_SIGNAL_ADDED_LINE_THRESHOLD = 4
...
# MINER-EDITABLE WITH BOUNDARIES: Keep all requests pointed at api_base/api_key
# supplied by solve(); the validator proxy rewrites model/sampling server-side.
def chat_completion(
...
# MINER-EDITABLE: bash tool surface used inside the task repo.
