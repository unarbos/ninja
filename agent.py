...
# v12: restored to 8192 after duel data showed minimax-m2.7 needs the headroom
# to express both reasoning and <command> blocks in a single response. Capping
# at 4096 caused command-truncation on the new solver model: responses hit the
# cap mid-explanation and never emitted commands, producing empty patches with
# 0 tool_calls despite 9+ LLM round-trips. The earlier rationale (deepseek-v4
# rambling to the cap) is now handled by tighter inner-attempt wall-clock plus
# the safety-net wrapper, not by per-request token caps.
...
MAX_STEP_RETRIES = 2
WALL_CLOCK_BUDGET_SECONDS = 180.0  # v12 (timeout-safe): cut to 180 so we leave 120s margin per attempt and the safety-net wrapper can salvage on-disk patches before the validator's per-round soft cap. Adopted from king7 v43; production data shows challenger time-out rate dominates round losses on the new minimax solver.
...
# v12 (jupiter): when truncating compiler/test output, the actionable error is
# almost always at the tail (final stack frame, last failure summary). The
# default mid-truncation chops it out, leaving the model the noisy header but
# not the diagnostic. When markers like "Traceback", "panic:", "FAIL\t" appear,
# drop the head instead so the tail survives intact.
_TRACEBACK_MARKERS = (
...
def get_patch(repo: Path) -> str:
    exclude_pathspecs = [
        ...
        ":(exclude,glob)**/*.o",
        ":(exclude,glob)**/*.a",
        # lockfiles (regenerated by package managers, not what the issue asked for)
    ...
    cleaned = _strip_off_scope_path_blocks(cleaned)
    return _strip_low_signal_hunks(cleaned)


def _strip_off_scope_path_blocks(diff_output: str) -> str:
    """Drop file blocks whose target path matches the never-allowed skip set.

    Backstop for cases where the model edited a tracked binary, lockfile, or
    build artifact via shell commands that bypass git pathspec filtering
    (for example, an `npm install` that regenerates a tracked lockfile, or a
    `pip install` that drops a wheel into a tracked cache directory). Same
    skip set as _should_skip_patch_path; applied to the unified diff after
    the mode-only pass and before the hunk-level low-signal strip.
    """
    ...
    for block in blocks:
        ...
        if _should_skip_patch_path(b_path):
            continue
        kept.append(block)
...
def _strip_off_scope_files_by_issue(diff_output: str, task_text: str) -> str:
    """Drop file blocks not related to the task description.

    A file block counts as in-scope if any of:
      - its path is mentioned in the task text
      - its filename or stem (>=4 chars) appears in the task text
      - its added lines reference at least one task-extracted identifier symbol
      - its stem matches an in-scope file's stem (catches companion .css/.test/etc.)

    Conservative: the strip ONLY fires when at least one in-scope file is
    present in the diff. If every file looks off-scope by these heuristics,
    the original diff is returned unchanged on the assumption that the agent
    inferred the right files via signals we cannot see.
    """
    if not diff_output.strip() or not (task_text or "").strip():
        return diff_output

    mentioned_paths = {p.strip("./") for p in _extract_issue_path_mentions(task_text)}
    issue_symbols = list(_extract_issue_symbols(task_text))
    issue_text_lower = task_text.lower()

    blocks = re.split(r"(?=^diff --git )", diff_output, flags=re.MULTILINE)
    parsed: List[Tuple[Optional[str], str]] = []
    for block in blocks:
        if not block.startswith("diff --git "):
            parsed.append((None, block))
            continue
        first_line = block.split("\n", 1)[0]
        m = re.match(r"^diff --git a/(.+?) b/(.+?)$", first_line)
        if not m:
            parsed.append((None, block))
            continue
        parsed.append((m.group(2).strip(), block))

    def _is_in_scope(path: Optional[str], block: str) -> bool:
        if not path:
            return True
        ...
        if name_lower and name_lower in issue_text_lower:
            return True
        stem_lower = Path(path).stem.lower()
        if len(stem_lower) >= 4 and stem_lower in issue_text_lower:
            return True
        if issue_symbols:
            added_text = "\n".join(
            ...
            if any(sym in added_text for sym in issue_symbols):
                return True
        return False

    classified = [(p, blk, _is_in_scope(p, blk)) for p, blk in parsed]

    # Companion-stem boost: also keep files whose stem matches an in-scope
    # file's stem (catches `foo.css` next to `foo.tsx`, `tests/test_X.py`
    # next to `X.py`, generated `X.d.ts` next to `X.ts`, etc.).
    in_scope_stems = {
        Path(p).stem.lower()
        for p, _, in_scope in classified
        if p and in_scope and len(Path(p).stem) >= 4
    }
    if in_scope_stems:
        boosted: List[Tuple[Optional[str], str, bool]] = []
        for p, blk, in_scope in classified:
            if not in_scope and p:
                stem = Path(p).stem.lower()
                base_stem = re.sub(r"^test[_-]", "", stem)
                if stem in in_scope_stems or base_stem in in_scope_stems:
                    in_scope = True
            boosted.append((p, blk, in_scope))
        classified = boosted

    # Conservative gate: skip the strip entirely if NO file looked in-scope.
    if not any(in_scope for p, _, in_scope in classified if p):
        return diff_output

    kept = [block for _, block, in_scope in classified if in_scope]
    result = "".join(kept)
    if diff_output.endswith("\n") and result and not result.endswith("\n"):
        result += "\n"
    return result
...
def _finalize_patch_for_validator(
    result: Dict[str, Any], task_text: str
) -> Dict[str, Any]:
    """Apply final-only filters to the patch before returning to the validator.

    Strips off-scope file blocks (files neither mentioned in the issue nor
    referencing an issue-extracted symbol nor sharing stem with such a file).
    Off-scope churn is the most-cited LLM judge penalty after incompleteness;
    this final pass drops it without affecting in-loop refinement decisions
    (which still see the full patch via get_patch()).
    """
    patch = result.get("patch", "") or ""
    if not patch.strip():
        return result
    cleaned = _strip_off_scope_files_by_issue(patch, task_text)
    if cleaned == patch:
        return result
    out = dict(result)
    out["patch"] = cleaned
...
# Path-skip sets used by _should_skip_patch_path and _strip_off_scope_path_blocks.
# Lockfiles, build artifacts, and binary objects are persistent sources of
# "unrelated churn" complaints in the LLM judge: the model sometimes runs
# install/build commands that regenerate these files, and the resulting hunks
# never advance the task but always cost score. Skip them at multiple layers
# (git pathspec exclude in get_patch, untracked filter in _should_skip_patch_path,
# and a final post-pass in _strip_off_scope_path_blocks) so they cannot leak in
# via any code path.
_SKIP_LOCKFILE_NAMES = frozenset({
    "package-lock.json", "yarn.lock", "pnpm-lock.yaml", "bun.lockb", "bun.lock",
    "cargo.lock", "poetry.lock", "pipfile.lock", "gemfile.lock", "composer.lock",
    "go.sum", "flake.lock", "uv.lock",
})
_SKIP_BINARY_SUFFIXES = frozenset({
    ".pyc", ".pyo", ".so", ".dylib", ".dll", ".exe",
    ".o", ".a", ".class", ".jar", ".war", ".aar", ".obj", ".pdb",
})
_SKIP_DIR_PARTS = frozenset({
    "__pycache__", ".pytest_cache", "node_modules", ".git",
    ".next", "dist", "build", "target", ".venv", "venv",
    "vendor", ".cache", "coverage", ".gradle", ".mvn",
})


def _should_skip_patch_path(relative_path: str) -> bool:
    """True for paths that should never appear in a miner-submitted patch.

    Expanded from the original (which only checked .pyc + a few cache dirs) to
    cover lockfiles, build artifacts, and binary objects. The LLM judge
    consistently penalises these as 'unrelated churn' even when the change is
    technically meaningful (e.g. a regenerated lockfile after a dependency
    edit), so the safer policy is to never let them reach the validator.
    """
    path = Path(relative_path)
    if path.suffix.lower() in _SKIP_BINARY_SUFFIXES:
        return True
    if path.name.lower() in _SKIP_LOCKFILE_NAMES:
        return True
    parts_lower = {part.lower() for part in path.parts}
    return bool(parts_lower & _SKIP_DIR_PARTS)
...
# v12 (jupiter): aggressive previous stopword set killed many real identifier
# names ("config", "context", "parse", "build", "class", "file", "field"...).
# Restricted to ultra-common English words only, so identifier tokens that
# happen to be normal English words still feed _symbol_grep_hits and surface
# the right files in preloaded context.
_SYMBOL_STOP = {
...
def build_budget_pressure_prompt(step: int) -> str:
    ...
    # v12: late-step minimax-style hint. When the model has used 6+ steps
    # without producing a patch, its responses are likely consuming too
    # many tokens on planning before reaching <command> blocks, getting
    # truncated mid-thought. Force a strict format-first response.
    return (
        "FORMAT EMERGENCY: 6+ steps used without a patch. "
        "Your next response MUST start with a <command> block as the very first thing. "
        "Skip all planning, explanation, and <plan> blocks. "
        "Just emit one or more <command>...</command> blocks that edit the most likely file, "
        "then end with <final>summary</final>. Anything else wastes the response budget."
    )
...
_MULTISHOT_MIN_ATTEMPT_RESERVE = 180.0  # v12 (king7 v43): never start a retry unless we have at least one full attempt budget (180s) left, so the retry can't push us past the validator's per-round soft cap.


# v12 (jupiter): observation-loop hint for compiler/test error locations.
# When a build or test command fails with file:line markers, the model often
# wastes the next turn re-grepping for "where did it fail?" Extracting those
# (path, line) pairs and surfacing them at the bottom of the observation
# saves a step and points the model directly at what to edit.
_ERR_LOC_PATTERNS = [
...
# v12 (jupiter): multi-shot memo. The previous wrapper retried with the IDENTICAL
# user prompt, so attempt 2 reproduced attempt 1's mistakes. Build a compact
# memo summarising attempt 1's actual outcome (which files were touched, how
# many substantive lines, which mentioned paths were missed, last failed
# command) and prepend it to attempt 2's user message so the model can take
# a different angle. Critical when individual attempts cost 60s+ on minimax.
...
def _build_multishot_memo(result: Dict[str, Any], task_text: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "attempt1_unaddressed_paths": _uncovered_required_paths(patch, task_text),
...
# -----------------------------
# Empty-patch failure-mode 2 fallback
# -----------------------------
#
# Two distinct failure modes produce empty patches:
#
#   Mode 1 ("rambling timeout"): the model burns 100+ seconds per call writing
#     verbose multi-step plans against the full max_tokens cap, and exhausts
#     the inner attempt's wall-clock budget before emitting any <command>
#     block. Attacked by lowering DEFAULT_MAX_TOKENS and the in-loop adaptive
#     reduction in _solve_attempt.
#
#   Mode 2 ("model errors"): the proxy returns repeated transport failures or
#     short non-JSON responses; the inner loop hits MODEL_ERROR_GIVE_UP
#     after 3 consecutive errors and returns an empty patch. The standard
#     multi-shot retry runs the same prompt chain and tends to hit the same
#     errors, so it does not recover this case.
#
# This helper is the dedicated Mode 2 escape hatch: a single chat call with a
# hard 45-second timeout, max_tokens 1024, no retries, and a stripped-down
# prompt that bypasses the full planning/refinement chain. Quality is
# secondary; the goal is converting a guaranteed empty-patch loss into any
# substantive edit so the round can be scored above zero.
...
def _solve_emergency_single_shot(**kwargs: Any) -> Dict[str, Any]:
    """Single-call fallback for Empty Mode 2 (model-error empty patches)."""
    ...
    try:
        ...
        try:
            ...
            patch = get_patch(repo) if repo is not None else ""
            return AgentResult(
                patch=patch, logs=_safe_join_logs(logs),
                steps=0, cost=0.0, success=bool(patch.strip()),
            ).to_dict()
...
# MINER-EDITABLE: validator entry point. v12: thin shell that delegates to
# _solve_with_safety_net so any uncaught exception still returns the on-disk
# patch instead of propagating an empty result. The signature is preserved
# exactly to satisfy the validator and PR Scope Guard.
def solve(
    repo_path: str,
    issue: str,
    ...
    v12 (king7 v43 + jupiter PR514 + v11a P2): wrapped in a safety-net so
    any uncaught exception (timeout, network, OOM) returns the on-disk patch
    state instead of propagating empty. Production duel data on minimax-m2.7
    showed ~16% empty-patch rate dominated by `time_limit_exceeded` and
    `solver_error`; the safety net converts those crashes into "whatever
    partial work survived on disk" so we never ship a literal empty patch.
    """
...
def _solve_with_safety_net(**kwargs: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        _multishot_total_budget = 400.0  # v12 (king7 v43)
        _multishot_initial_head = (
            _multishot_capture_head(_multishot_repo_obj) if _multishot_repo_obj else None
        )
        ...
        # v12 (jupiter): smarter retry condition. A 1-2 line surgical fix that
        # actually covers the issue-mentioned paths is correct -- don't throw
        # it away for retry. Only retry on (a) empty patch, or (b) low-signal
        # patch that ALSO doesn't cover the required paths.
        _covered_required = bool(_patch1.strip()) and not _uncovered_required_paths(_patch1, task_text)
        _should_retry = (not _patch1.strip()) or (
            _n1 < _MULTISHOT_LOW_SIGNAL_THRESHOLD and not _covered_required
        )
        if not _should_retry:
            _result1["multishot_attempts"] = 1
            return _finalize_patch_for_validator(_result1, task_text)

        _elapsed = time.monotonic() - _multishot_started

        # v12: empty-patch escape. Triggers when attempt 1 produced no patch AND
        # any of the structural-failure signals fire (model errors hit retry
        # cap; gave up; OR no observations were ever produced -- meaning the
        # model never emitted a usable <command> block, the hallmark Mode A
        # failure on minimax-m2.7's verbose responses). Switch to a single
        # tight one-shot prompt that bypasses the planning chain.
        _attempt1_logs = _result1.get("logs", "") or ""
        _no_observations = (
            not _patch1.strip()
            and "OBSERVATION " not in _attempt1_logs
            and "===== STEP " in _attempt1_logs
        )
        _attempt1_was_error_failure = (
            not _patch1.strip()
            and (
                _attempt1_logs.count("MODEL_ERROR") >= 2
                or "MODEL_ERROR_GIVE_UP" in _attempt1_logs
                or _no_observations
            )
        )
        if _attempt1_was_error_failure and (_multishot_total_budget - _elapsed) > 60.0 and _multishot_repo_obj is not None:
            ...
            if _multishot_count_substantive(_patch_emergency) > 0:
                ...
                return _finalize_patch_for_validator(_result_emergency, task_text)
            _elapsed = time.monotonic() - _multishot_started
        ...
        if _multishot_repo_obj is not None:
            _multishot_revert(_multishot_repo_obj, _multishot_initial_head)
        # v12 (jupiter): pass attempt 1 memo to attempt 2 so it can take a
        # different angle instead of repeating the same path.
        _retry_kwargs = dict(kwargs)
        _retry_kwargs["_multishot_memo"] = _build_multishot_memo(_result1, task_text)
        ...
        if _n2 >= _n1:
            ...
            return _finalize_patch_for_validator(_result2, task_text)
        ...
        # v12 (king7 v43): ANY uncaught exception from the multi-shot body
        # should not propagate. Salvage whatever patch is on disk right now,
        # apply the off-scope finalize, and return as a normal result. Don't
        # catch BaseException -- let SystemExit/KeyboardInterrupt do their
        # thing so the validator can clean-kill the process.
        salvaged = ""
        try:
            if _multishot_repo_obj is not None:
        ...
        return _finalize_patch_for_validator(_crashed, task_text)
...
def _solve_attempt(**kwargs: Any) -> Dict[str, Any]:
    ...
    # v12 (jupiter): when the multi-shot wrapper retries, it can pass a memo
    # describing what attempt 1 produced. We prepend that memo to the user
    # prompt so the model knows not to repeat the same approach.
    ...
    def maybe_queue_refinement(assistant_text: str) -> bool:
        ...
        v12 (jupiter): judge-half gates (coverage/criteria/test) fire BEFORE
        cursor-half gates (polish/syntax) so the two-turn refinement cap is
        spent on the highest-leverage signals first. The combined Score is
        50% Cursor similarity + 50% LLM judge; coverage/criteria/test gates
        directly attack the LLM-judge "missing N of M" complaint that drove
        most king losses in production duels. Polish is largely redundant
        with v11a's _strip_off_scope_files_by_issue at the finalize layer.
        """
        ...
        # Hail-mary is exempt from the total-refinement cap: it is the only
        # path out of a guaranteed-zero empty-patch result.
        if not patch.strip():
        ...
        # v12: syntax-fix is exempt from the total-refinement cap (cap-exempt
        # like hail-mary). Reasoning: jupiter's reorder pushes syntax to the
        # end of the chain, so under the 2-turn cap it would never fire even
        # when the patch has a real parse error. A patch that doesn't parse
        # scores low on BOTH halves of the combined score (LLM judge sees
        # broken code, similarity comparison may fail to apply), so always
        # allow one syntax-fix turn when _check_syntax detects a real error.
        if syntax_fix_turns_used < MAX_SYNTAX_FIX_TURNS:
            syntax_errors_pre = _check_syntax(repo, patch)
            if syntax_errors_pre:
                syntax_fix_turns_used += 1
                queue_refinement_turn(
                    assistant_text,
                    build_syntax_fix_prompt(syntax_errors_pre),
                    "SYNTAX_FIX_QUEUED (cap-exempt):\n  " + "\n  ".join(syntax_errors_pre),
                )
                return True
        ...
        # Companion-test execution gate. If any edited file has a partner test
        # that actually fails, surface the failure tail to the model as one
        # fix turn. This is the only refinement step in the chain backed by a
        # real runner rather than heuristics, and the LLM judge rewards
        # passing tests.
    ...
    try:
        ...
        # v12 (jupiter): prepend prior-attempt memo when the multi-shot
        # wrapper is asking for a different angle on retry.
        ...
        for step in range(1, max_steps + 1):
            ...
            # v12: adaptive halving removed. Real duel data on minimax-m2.7
            # showed that when the agent is "stuck", the model needs MORE
            # room to lay out reasoning + commands, not less. The previous
            # halving rule (step >= 5 with no patch -> max_tokens // 2)
            # produced direct empty-patch failures: responses hit the lower
            # cap mid-explanation and never reached the <command> block.
            ...
            if observations:
                observation_text = "\n\n".join(observations)
                # v12 (jupiter): if any command output contains compiler/test
                # error location markers, surface them as a bulleted hint at
                # the bottom of the observation. Skip when we already
                # declared success so we don't override the success message.
            ...
            if not get_patch(repo).strip() and step in {2, 4, 6}:
                # v12: added step 6 trigger to fire the format-emergency
                # prompt for late-step empty-patch cases on minimax-m2.7.
