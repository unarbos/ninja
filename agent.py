...
def _strip_mode_metadata_lines(diff_output: str) -> str:
    ...
    applied at solve startup: the setting prevents the lines from being
    generated; this strip removes any that survive (older git or alternate
    diff backend). Pure text-level: removes only metadata lines, never
...
def _solve_attempt(**kwargs: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        # lines, so they only widen cursor-similarity distance.
        try:
            subprocess.run(
