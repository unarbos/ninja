...
def _compose_initial_user_prompt(
    task: str,
    repo_summary: str,
    prelocalized: str = "",
    ...
    if prelocalized.strip() and not reference_hint.strip():
        parts.extend(["", "Files referenced by the task (pre-loaded):", "", prelocalized])
    parts.extend([
        "",
        "Strategy:",
    ])
...
def prelocalize_files_from_issue(repo: Path, task: str, max_files: int = 8, max_bytes_per_file: int = 4000) -> str:
    candidates: List[str] = []
    seen: set[str] = set()
    for match in _FILE_HINT_RE.findall(task):
        if match in seen:
            continue
        seen.add(match)
...
def extract_acceptance_criteria(task: str, max_items: int = 16) -> List[str]:
    """Pull bulletted/numbered acceptance items out of the task text."""
    section_re = re.compile(
        r"(?:acceptance\s+criteria|requirements|tasks?|todo)\s*:?\s*\n([\s\S]*?)(?:\n\n|\n(?=[A-Z][^a-z\n])|\n(?=##)|$)",
        re.IGNORECASE,
    )
    m = section_re.search(task)
    block = m.group(1) if m else task
...
def extract_identifiers(task: str, max_items: int = 12) -> List[str]:
    found: List[str] = []
    seen: set[str] = set()
    ...
    for regex in (_IDENT_BACKTICK_RE, _IDENT_PASCAL_RE, _IDENT_CAMEL_RE, _IDENT_SNAKE_RE):
        for match in regex.findall(task):
            ident = match.strip()
...
def solve(
    ...
    try:
        ...
        messages: List[Dict[str, str]] = [
            ...
            {
                "role": "user",
                "content": _compose_initial_user_prompt(
                    issue,
                    repo_summary,
