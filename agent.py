...
TIME IS CRITICAL. Minimize exploration:
...
## Multi-requirement discipline

When the issue has multiple bullets or numbered items:
1. List ALL requirements in your <plan> block before editing
2. Address EVERY requirement in your FIRST edit response
3. Missing even ONE requirement costs more than a slightly imperfect fix
...
If the patch is complete, respond with <final>summary</final>. Otherwise make a surgical edit NOW using sed or python -c. Do not re-read files you already have."""
...
def build_budget_pressure_prompt(step: int) -> str:
    ...
    return (
            "URGENT: still no patch. "
            "Your next command MUST make a code change RIGHT NOW — even a best-effort minimal edit to the most obvious location. "
            "Do not read files or run tests until after a patch exists. "
            "Use `sed -i` or a python one-liner to make the targeted edit NOW. An imperfect patch beats an empty one."
    )
...
def build_hail_mary_prompt(issue_text: str) -> str:
    ...
    return (
        ...
        "COMMON FIX PATTERNS (pick the one matching the issue):\n"
        "  - Off-by-one: change < to <=, or i to i+1\n"
        "  - Missing return: add return at end of function\n"
        "  - Wrong condition: flip == to !=, or and to or\n"
        "  - Missing import: add the import at the top\n"
        "  - Missing field: add the field to dict/class/object"
    )
        ...
        for step in range(1, max_steps + 1):
            ...
            if observations:
                ...
                if not success and get_patch(repo).strip():
                    observation_text += (
                        "\n\nPatch exists. Run ONE targeted test to verify, then emit <final> immediately. Do NOT explore further."
                    )
                elif not success:
                    observation_text += (
