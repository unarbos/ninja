...
# v2.4: the eight separate refinement caps below are replaced by a single
# LLM-judge gate. The previous ladder ran 7 priority-ordered checks
# (hail-mary / polish / syntax / test / coverage / criteria / self-check)
# with a total-cap on top, ~130 lines of keyword matching, and a
# dedicated runner-process gate. Half of those checks were
# keyword-overlap heuristics that fire when the model picked the "wrong"
# file extension or used a synonym; another (`self_check`) just asked
# the model to review its own work. We replace all of it with one
# acceptance-judge call (see ``judge_acceptance``).
MAX_ACCEPTANCE_CHECK_TURNS = 1   # at most one judge call per solve
ACCEPTANCE_CHECK_MAX_ISSUE_CHARS = 4000
ACCEPTANCE_CHECK_MAX_RESPONSE_CHARS = 3000
ACCEPTANCE_CHECK_MAX_PATCH_CHARS = 6000
ACCEPTANCE_CHECK_MAX_TOKENS = 400  # judge response is short: VERDICT + REASON
...
# refinement ladder; the ladder is now a single LLM-judge gate (see
# ``judge_acceptance`` + the two acceptance prompts below). Hail-mary,
# polish, coverage, criteria, self-check are all subsumed into what the
# judge is asked to evaluate.


# -----------------------------
# v2.4: LLM acceptance judge
# -----------------------------

ACCEPTANCE_CHECK_SYSTEM = (
    "You are a strict, concise code-review judge. You read an issue, the "
    "agent's last response, and the diff the agent produced. Decide whether "
    "the patch correctly and completely addresses the issue.\n\n"
    "Respond on exactly two lines, in this format:\n"
    "VERDICT: PASS\n"
    "REASON: <one short sentence>\n\n"
    "Or, if the patch is wrong / incomplete / off-target:\n"
    "VERDICT: FAIL\n"
    "REASON: <one concrete actionable sentence describing what is missing "
    "or wrong, that the agent can use to fix it in one more round>\n\n"
    "Be permissive on stylistic differences. Be strict on correctness, "
    "completeness vs the issue's explicit requirements, and obvious "
    "regressions. If you cannot tell, prefer PASS — the validator's "
    "similarity score will catch low-effort patches independently."
)


def build_acceptance_check_prompt(
    issue_text: str,
    last_response: str,
    patch: str,
) -> str:
    """User message for the acceptance judge. Quotes the issue, the agent's
    final assistant turn, and the patch — each truncated to a per-section
    budget so the judge call stays bounded regardless of how verbose the
    agent was. Demands the ``VERDICT:`` + ``REASON:`` format described in
    ``ACCEPTANCE_CHECK_SYSTEM``."""
    issue_short = _truncate(issue_text, ACCEPTANCE_CHECK_MAX_ISSUE_CHARS)
    response_short = _truncate(last_response, ACCEPTANCE_CHECK_MAX_RESPONSE_CHARS)
    patch_short = _truncate(patch, ACCEPTANCE_CHECK_MAX_PATCH_CHARS)
    return (
        "## Issue\n\n"
        f"{issue_short}\n\n"
...
def build_acceptance_fix_prompt(reason: str, issue_text: str) -> str:
    """User message queued back to the agent on FAIL. Quotes the judge
    verbatim and demands a final response — single refinement turn, no
    further chain of nudges (the ladder is gone)."""
    issue_short = issue_text[:1500]
    reason_short = (reason or "(no reason given)").strip()[:600]
    return (
        "Acceptance review FAILED. The judge wrote:\n\n"
        f"  {reason_short}\n\n"
        "Address that gap in your next response — make whatever edit is "
        "needed (sed -i, python -c, or a heredoc), then end with "
        "<final>summary</final>. Do NOT add unrelated scope and do NOT "
        "rewrite parts of the patch the judge did not flag.\n\n"
        "Task (for reference):\n"
        f"{issue_short}\n"
    )
...
def judge_acceptance(
    ...
    *,
    ...
    """v2.4: ask the LLM whether the current patch is acceptable.

    Returns ``(passed, reason)``.

    Permissive on every error: empty response, unparseable verdict, and
    network / provider exceptions all map to ``(True, "...")`` so a flaky
    judge can never block shipping. The validator's similarity score
    independently penalises low-effort patches; the judge is an additional
    correctness signal, never a hard gate against returning *something*.
    """
    if not patch.strip():
        return True, "empty patch — judge skipped"
    try:
        response_text, _cost, _raw = chat_completion(
            messages=[
                {"role": "system", "content": ACCEPTANCE_CHECK_SYSTEM},
                {
                    "role": "user",
                    "content": build_acceptance_check_prompt(
            ...
            api_base=api_base,
            api_key=api_key,
            max_tokens=ACCEPTANCE_CHECK_MAX_TOKENS,
        )
    except Exception as exc:
        return True, f"judge unreachable: {type(exc).__name__}: {str(exc)[:120]}"

    if not response_text or not response_text.strip():
        return True, "judge returned empty response"

    verdict_match = re.search(
        r"^\s*VERDICT\s*:\s*(PASS|FAIL)\b", response_text, re.IGNORECASE | re.MULTILINE
    )
    reason_match = re.search(
        r"^\s*REASON\s*:\s*(.+?)\s*$", response_text, re.IGNORECASE | re.MULTILINE
    )
    reason = reason_match.group(1) if reason_match else response_text.strip()[:300]

    if verdict_match is None:
        return True, f"judge response unparseable; reason='{reason[:120]}'"
    return verdict_match.group(1).upper() == "PASS", reason
...
def _solve_attempt(**kwargs: Any) -> Dict[str, Any]:
    ...
    acceptance_check_turns_used = 0  # v2.4: replaces 8 separate counters
    ...
    def maybe_queue_refinement(assistant_text: str) -> bool:
        """v2.4 single-shot acceptance gate.

        Replaces the 7-check ladder (hail-mary / polish / syntax / test /
        coverage / criteria / self-check) with one LLM-judge call. Returns
        True when the loop should continue (judge said FAIL and we queued
        a fix turn); False means the caller can declare success.

        Permissive on every judge error — see ``judge_acceptance`` — so a
        flaky judge can never block shipping. Total budget is one judge
        call per solve (``MAX_ACCEPTANCE_CHECK_TURNS``); after that the
        gate is silent and the model's ``<final>`` is honoured immediately.
        """
        nonlocal acceptance_check_turns_used
        ...
        if acceptance_check_turns_used >= MAX_ACCEPTANCE_CHECK_TURNS:
            return False
        acceptance_check_turns_used += 1

        passed, reason = judge_acceptance(
            issue_text=issue,
        ...
        queue_refinement_turn(
            assistant_text,
            build_acceptance_fix_prompt(reason, issue),
            f"ACCEPTANCE_FAIL_QUEUED:\n  {reason[:200]}",
        )
        return True
    ...
    try:
        ...
        messages: List[Dict[str, str]] = [
            ...
            {"role": "user", "content": _build_section_user_prompt(issue, repo_summary, preloaded)},
        ]
