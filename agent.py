...
_NOISE_DIRS = {
    ".github", ".gitlab", ".circleci", ".husky", ".vscode", ".idea",
...
def _split_diff_into_file_blocks(diff_text: str) -> List[List[str]]:
...
def _block_target_path(block: List[str]) -> Optional[str]:
    for ln in block:
        m = re.match(r"^\+\+\+ b/(.+)$", ln)
        if m:
            return m.group(1)
...
def _select_safe_recovery_changes(diff_text: str, issue_text: str, repo: Path) -> str:
    blocks = _split_diff_into_file_blocks(diff_text)
    if not blocks:
        return ""
    issue_paths = {p.strip("./") for p in _extract_issue_path_mentions(issue_text)}
    expected = set(_expected_files_from_issue(issue_text, repo))
    kept: List[List[str]] = []
    for block in blocks:
        path = _block_target_path(block)
        if not path or _is_noise_path(path):
            continue
        kept.append(block)
    if issue_paths or expected:
        prioritized: List[List[str]] = []
        rest: List[List[str]] = []
        for block in kept:
            path = _block_target_path(block) or ""
            base = path.split("/")[-1]
            if (path in issue_paths or path in expected or
                    base in issue_paths or base in expected):
                prioritized.append(block)
            else:
                rest.append(block)
        kept = prioritized + rest
    out_lines: List[str] = []
    for block in kept[:20]:
...
def _try_apply_recovery(repo: Path, recovery_diff: str) -> int:
    if not recovery_diff.strip():
        return 0
    try:
        proc = subprocess.run(
            ["git", "-C", str(repo), "apply", "--whitespace=nowarn", "-"],
            input=recovery_diff, text=True, capture_output=True, timeout=30,
        )
        if proc.returncode == 0:
            return 1
    except Exception:
        return 0
    blocks = _split_diff_into_file_blocks(recovery_diff)
    applied = 0
    for block in blocks:
        path = _block_target_path(block)
        if not path or _is_noise_path(path):
        ...
        try:
            proc = subprocess.run(
                ["git", "-C", str(repo), "apply", "--whitespace=nowarn", "-"],
                input=mini, text=True, capture_output=True, timeout=20,
            )
            if proc.returncode == 0:
                applied += 1
        except Exception:
...
def _classify_hunk_lines(hunk_body: List[str]) -> str:
    added: List[str] = []
    removed: List[str] = []
    for ln in hunk_body:
    ...
    if not added and not removed:
        return "blank"
    norm_a = "".join(s.strip() for s in added)
    norm_r = "".join(s.strip() for s in removed)
    if norm_a == norm_r:
        return "whitespace"
    only_blanks = all(not s.strip() for s in added + removed)
    if only_blanks:
        return "blank"
    only_comment = True
...
def _per_hunk_minimize(repo: Path) -> int:
    try:
        proc = subprocess.run(
    ...
    if proc.returncode != 0 or not proc.stdout.strip():
        return 0
    dropped_total = 0
    blocks = _split_diff_into_file_blocks(proc.stdout)
    for block in blocks:
        plus_path = _block_target_path(block)
        if not plus_path:
            continue
        if not (plus_path.endswith(".py") or plus_path.endswith(".js") or
                plus_path.endswith(".ts") or plus_path.endswith(".tsx") or
                plus_path.endswith(".jsx") or plus_path.endswith(".go") or
                plus_path.endswith(".java") or plus_path.endswith(".cpp") or
                plus_path.endswith(".c") or plus_path.endswith(".rs")):
            continue
        hunk_starts = [j for j, ln in enumerate(block) if ln.startswith("@@")]
        if not hunk_starts:
            continue
        header = block[:hunk_starts[0]]
        hunk_starts.append(len(block))
        hunks: List[List[str]] = []
        for j in range(len(hunk_starts) - 1):
            hunks.append(block[hunk_starts[j]:hunk_starts[j + 1]])
        for hunk in reversed(hunks):
            if len(hunk) < 2:
                continue
            kind = _classify_hunk_lines(hunk[1:])
            if kind in ("whitespace", "blank", "comment"):
                mini = "\n".join(header + hunk) + "\n"
                try:
                    res = subprocess.run(
                        ["git", "-C", str(repo), "apply", "-R", "--whitespace=nowarn", "-"],
                        input=mini, text=True, capture_output=True, timeout=15,
                    )
                    if res.returncode == 0:
                        dropped_total += 1
                except Exception:
...
def solve(
    ...
    try:
        ...
        if history_hint:
            ...
            try:
                safe_recovery = _select_safe_recovery_changes(history_hint, issue, repo)
                if safe_recovery:
                    prepass_applied = _try_apply_recovery(repo, safe_recovery)
                    if prepass_applied:
                        logs.append(
                            f"HISTORY_PREPASS:\nApplied {prepass_applied} file change(s) "
                            "from local history as starting point; agent will refine."
                        )
            except Exception:
                logs.append("HISTORY_PREPASS_ERROR:\n" + traceback.format_exc())
        ...
        if prepass_applied:
            initial_user = initial_user + (
                "\n\nA starting-point patch has been applied to the working tree "
                "based on related history in this repo. Run `git diff` first to "
                "see what is already staged. Verify that the applied changes fully "
                "address the issue's acceptance criteria; revise, extend, or "
                "minimize as needed. Do NOT mention the starting-point hint in "
        ...
        if patch.strip():
            try:
                ...
                dropped_hunks = _per_hunk_minimize(repo)
                if dropped_hunks:
                    logs.append(f"PER_HUNK_MINIMIZE:\nDropped {dropped_hunks} trivial hunk(s).")
            except Exception:
