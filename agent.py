...
SYSTEM_PROMPT = """You are a high-quality coding agent running inside a repository.

You must fix the issue by editing files in the repo. You have a tight wall-clock
budget, so produce a minimal, targeted, high-quality patch quickly instead of
exhaustively exploring or over-engineering.

CRITICAL: Your patches are scored against professional baseline solutions. Focus
on correctness and conciseness—avoid unnecessary changes, refactoring, or
formatting that isn't directly required by the issue.
...
Rules:
- Work directly in the repository. Minimal, targeted edits only.
- Match professional solution quality: concise, correct, and focused.
- If relevant file snippets are preloaded, edit those files immediately; do NOT
  re-read them (re-reading burns budget).
- If target is unclear, run ONE or TWO focused grep/snippet commands, then edit.
  Avoid broad inspection loops.
- By your second response you should be editing code, not exploring.
- Emit ALL independent file-edit commands in one response; do NOT split edits
  across multiple turns.
- Avoid generating, minified, binary, lock, or vendored file content.
- Make edits immediately once the required code is identified.
- Run ONLY the cheapest verification (syntax/type check for touched files).
  Skip full builds, broad test suites, or full installs.
- If a verification command is slow or dependencies are missing, keep the patch
  and finish—do NOT spend budget on setup.
- After a focused patch exists plus one cheap verification, finish immediately.
- Do not use sudo, delete the repo, access secrets, or make external network calls.
- Do not modify hidden tests, evaluator files, or create unrelated formatting churn.
- Do not stop after explaining; actually edit the code. Action, not description.
- Avoid chmod/mode changes and unnecessary refactoring.
- You may use python, sed, cat, grep, find, pytest, npm, etc. if available.
- QUALITY > SPEED: A correct, minimal patch is worth more than a bloated one.
"""
...
These files have already been read for you. Re-reading them burns your budget.
Edit them directly with surgical precision—make only the necessary changes to
fix the issue. Avoid reformatting, refactoring, or over-engineering.
"""

    return f"""Fix this issue with minimal, targeted changes:
...
APPROACH:
1. If preloaded snippets show target code, edit them immediately (no re-reading).
2. If unclear, run ONE or TWO focused grep/search commands, then edit.
3. Make all independent file edits in a SINGLE response (no multi-turn edits).
4. Do NOT run full test suites before patching. After patch exists, run only ONE
   cheap verification (syntax/type check for touched files).
5. Finish immediately after patch + one verification. Quality over Polish.

Your patch will be scored against professional baseline solutions. Conciseness,
correctness, and minimal scope matter most.
"""
...
def build_budget_pressure_prompt(step: int) -> str:
    if step < 4:
        return """Budget check: you must start editing now. Use the issue + observed snippets to make targeted edits to the most likely file(s). No more exploration. Minimal scope only."""
    return """HARD LIMIT: Create a focused, minimal code change NOW that addresses the core issue. Do NOT inspect more files or run full tests. Make the patch, verify syntax/types only, then finish."""
        ...
        for step in range(1, max_steps + 1):
            ...
            for command_index, command in enumerate(command_batch, 1):
                ...
                if step >= 4 or command_index > 1:
                    ...
                    if patch.strip() and step >= 8 and _looks_like_patch_review_command(command, result):
                        ...
                        success = True
                        break
                    # IMPROVEMENT: Exit early if patch exists after syntax/type check passes
                    if patch.strip() and step >= 3 and _looks_like_syntax_check_command(command, result):
                        if result.exit_code == 0:
                            logs.append("\nAUTO_STOP:\nPatch exists and syntax/type check passed.")
                            success = True
                            break
            ...
            if observations:
                ...
                if not success and get_patch(repo).strip():
                    observation_text += (
                        "\n\nPatch now exists. Do ONE of:\n"
                        "1. If patch is complete, respond with <final>summary</final>\n"
                        "2. If more edits needed, send ALL remaining independent "
                        "file-edit commands in ONE response.\n"
                        "3. Run ONE quick syntax/type check (no full test suites), then "
                        "respond with <final>summary</final>.\n"
                        "Do NOT spend multiple turns. Do NOT over-engineer or refactor."
                    )
                elif not success:
                    observation_text += (
...
def _looks_like_syntax_check_command(command: str, result: CommandResult) -> bool:
    """Check if command is a quick syntax/type check (not a full test suite)."""
    if result.exit_code != 0:
        return False
    lowered = command.lower().strip()
    patterns = [
        r"\bpython\d*(\.\d+)?\s+-m\s+py_compile\b",
        r"\bpython\d*(\.\d+)?\s+-m\s+ast\b",
        r"\bnpx\s+tsc\s+--noEmit\b",
        r"\btsc\s+--noEmit\b",
        r"\bruff\s+check\b",
        r"\bpylint\s+--errors-only\b",
        r"\bflake8\b",
        r"\brustfmt\s+--check\b",
        r"\bcargo\s+check\b",
        r"\bgo\s+fmt\b",
        r"\bgo\s+vet\b",
    ]
