...
WALL_CLOCK_TIMEOUT_SECONDS = int(os.environ.get("AGENT_WALL_CLOCK_TIMEOUT_SECONDS", "540"))
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            if retry_after_header:
                try:
                    retry_after = max(1.0, float(retry_after_header))
                except ValueError:
                    retry_after = 1.0
    ...
    for index, ch in enumerate(text):
        ...
        if ch in ")]}":
            if not stack or stack[-1] != opener_for[ch]:
                return f"{relative_path}: brace balance error near char {index + 1}"
            stack.pop()
    if stack:
...
def solve(
    ...
    try:
        ...
        if reference_result and reference_result.applied_paths and not reference_result.pending_paths:
            patch = get_patch(repo)
        ...
        for step in range(1, max_steps + 1):
            ...
            step_time = time.time()
            if (step_time - start_time) >= WALL_CLOCK_TIMEOUT_SECONDS:
                patch = get_patch(repo)
                if patch.strip():
                    success = True
                break
