...
def _pull_issue_identifiers(issue_text: str, max_symbols: int = 24) -> List[str]:
    text = issue_text
    blocks: List[str] = []
...
def _grep_files_for_identifiers(repo: Path, symbols: List[str], limit: int = 12) -> List[str]:
    if not symbols:
        return []
    hits: dict = {}
...
def _llm_rank_files(
    *, repo: Path, issue_text: str,
    ...
    capped = allowed[:300]
    listing = "\n".join(capped)
    symbols = _pull_issue_identifiers(issue_text)
    grep_hits = _grep_files_for_identifiers(repo, symbols)
    symbol_block = ""
    if symbols:
...
def _expected_files_from_issue(issue_text: str, repo: Path) -> List[str]:
    ...
    symbols = _pull_issue_identifiers(issue_text)
    for f in _grep_files_for_identifiers(repo, symbols):
