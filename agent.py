...
If the issue describes changes across multiple layers (backend + frontend + navigation), your patch MUST include all layers. A patch that fixes only one layer fails.
If adding a page or view, also wire its URL route and update the nav or menu component.'''
...
def _detect_nav_route_gap(task_text: str, patch: str) -> list:
    """Return gap descriptions when a new page/component lacks route+nav wiring.

    Fires after criteria gate, at most once per solve.
    Targets navigation tasks where we add a page but forget the route or nav link.
    """
    import re as _re_nav
    nav_kws = ['route', 'navigation', 'navbar', 'nav', 'menu', 'page', 'url',
               'link', 'router', 'redirect', 'sidebar', 'path', 'endpoint']
    if sum(1 for w in nav_kws if w in task_text.lower()) < 2:
        return []
    touched = set(_re_nav.findall(r'^(?:\+\+\+|---) [ab]/(.+)$', patch, _re_nav.M))
    low = {f.lower() for f in touched}
    has_page = any('page' in f or 'view' in f or 'component' in f for f in low)
    has_route = any(p in f for f in low for p in
                    ['router', 'routes', 'navigation', 'nav', 'menu',
                     'sidebar', 'urls.py', 'app.py', 'index.ts'])
    if has_page and not has_route:
        return ["New page/component detected but no route or nav file touched"]
    return []


def build_nav_route_prompt(gaps: list) -> str:
    """Prompt the model to wire route + nav for a new page/component."""
    return (
        "Navigation gap: " + "; ".join(gaps[:2]) + "\n\n"
        "To complete the feature, add minimal wiring for:\n"
        "  (a) URL route registration (router/urls.py/routes file)\n"
        "  (b) Nav or menu link (sidebar, navbar, or top-level menu)\n"
        "Omitting either loses the round. End with <final>summary</final>."
    )
...
def _solve_attempt(**kwargs: Any) -> Dict[str, Any]:
    ...
    _nav_route_nudge_used = [False]  # v17: nav/route gap gate
    ...
    def maybe_queue_refinement(assistant_text: str) -> bool:
        ...
        # v17: nav/route wiring gate -- fires after criteria gate, once per solve
        if not _nav_route_nudge_used[0] and total_refinement_turns_used < MAX_TOTAL_REFINEMENT_TURNS:
            _nav_kws = ["route", "navigation", "navbar", "menu", "page", "url", "link"]
            if any(kw in issue.lower() for kw in _nav_kws):
                _nav_gaps = _detect_nav_route_gap(issue, patch)
                if _nav_gaps:
                    _nav_route_nudge_used[0] = True
                    total_refinement_turns_used += 1
                    queue_refinement_turn(assistant_text, build_nav_route_prompt(_nav_gaps), "NAV_ROUTE_GAP")
                    return True
