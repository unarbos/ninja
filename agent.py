...
def get_repo_summary(repo: Path) -> str:
    commands = [
        ...
        "find . -maxdepth 2 -type d | grep -v '.git' | sort | head -30 || true",
    ]
...
def build_polish_prompt(junk_summary: str) -> str:
    return (
        f"CLEANUP: Your patch has junk hunks that hurt diff quality:\n  {junk_summary}\n\n"
        "Revert ONLY those specific hunks using git checkout -- <file> or sed. "
        "Do NOT add new edits, refactor, reorder imports, or touch any other lines. "
        "If reverting cleanly is risky, respond <final>keeping patch as-is</final> immediately. "
        "Otherwise revert the junk and respond <final>cleaned up junk hunks</final>."
    )
...
SYSTEM_PROMPT = """You are an expert software engineer producing minimal, character-identical unified diffs.

GOAL: Produce the SMALLEST possible unified git diff that solves the issue. Every surplus line hurts your score.

You interact only by issuing bash commands. The environment runs your command and returns stdout/stderr.
...
STRATEGY - TWO PHASES:
Phase 1 (UNDERSTAND): Read the issue carefully. If preloaded snippets are provided, you already have context - skip to Phase 2. If not, run 1-2 focused grep/find/cat commands to locate target code. NEVER loop on inspection.
Phase 2 (EDIT): Make surgical edits immediately. By your 2nd response you MUST be editing files.

DISCIPLINE:
1. MINIMAL DIFFS: Match indentation, quotes, semicolons, trailing commas, blank lines, brace placement EXACTLY. Copy surrounding style character-for-character.
2. BREADTH-FIRST: When multiple files need changes, emit ALL independent file-edit commands in ONE response. Never split one patch across turns.
3. NO CHURN: Avoid whitespace-only edits, comment-only edits, import reorders, type annotation drive-bys, dead-code removal not requested, defensive checks not requested, and ANY unrelated refactors.
4. NO WASTE: Do not run broad test suites, full builds, or installs. Targeted py_compile/tsc --noEmit/pytest <one file> only. Do not dump huge generated/minified/binary/lock/vendored files.
5. STOP EARLY: After a focused patch and at most one cheap verification, finalize with <final>. Do not over-verify.
6. MATCH TOKENS: Use the exact identifiers, string tokens, and naming conventions the surrounding code already uses.
7. NO EXPLORATION LOOPS: If target is unclear after 1-2 grep commands, make your best focused edit anyway. You learn more from a failed edit than from reading more code.

FORBIDDEN:
- sudo, rm -rf, network calls (except validator inference proxy)
- Modifying hidden tests or evaluator files
- chmod/file mode changes
- Deleting the repository
- Explaining without actually editing
"""
...
APPROACH:
1. If preloaded snippets identify the target code, edit those files FIRST. Do not re-read them.
2. If target is unclear, run 1-2 focused grep/sed commands to locate it, then edit immediately.
3. Edit ALL files that need changes in the SAME response. Do not edit one file per turn.
4. After editing, run at most ONE cheap verification (py_compile, tsc --noEmit, or pytest <one file>).
5. Then finish with <final>summary</final>. Do not over-verify.

IMPORTANT: Produce the smallest possible diff. Match surrounding code style exactly. No unrelated changes.
"""
...
def build_self_check_prompt(patch: str, text: str) -> str:
    ...
    return (
        "SELF-CHECK: Quickly verify your patch is correct.\n\n"
        "Your patch:\n```diff\n"
        f"{truncated}\n```\n\n"
        "Task:\n"
        f"{text[:2000]}\n\n"
        "Check ONLY:\n"
        "1. Does the patch address the core issue? If not, emit one <command> to fix it.\n"
        "2. Any syntax errors? If yes, emit one <command> to fix.\n"
        "3. Any unrelated changes (whitespace, comments, refactors)? If yes, revert them.\n\n"
        "If good, respond: <final>OK</final>\n"
        "If fixing, emit corrective <command>(s) then <final>fixed</final>. "
        "Do NOT add new features or expand scope."
    )


def build_no_command_repair_prompt() -> str:
    return """ERROR: Your last response had no <command> or <final> block.

If done editing, respond: <final>summary</final>
Otherwise, issue a command NOW:
...
def build_budget_pressure_prompt(step: int) -> str:
    if step < 4:
        return """URGENT: No edits yet! Stop exploring. Use what you know to edit the most likely file NOW. A wrong edit is better than no edit - you can fix it next turn."""
    return """CRITICAL: Still no patch! Your VERY NEXT command must create a minimal edit in the most likely file. Do NOT inspect, do NOT search, do NOT run tests. EDIT NOW."""
        ...
        for step in range(1, max_steps + 1):
            ...
            if observations:
                ...
                if polish_pending:
                    ...
                    observation_text += (
                        "\n\nPatch exists. If more edits needed, send ALL remaining "
                        "file-edit commands in ONE response. Otherwise finish with <final>summary</final>."
                    )
                elif not success:
                    observation_text += (
                        "\n\nIf you have enough context, edit the target files NOW. "
                        "Send ALL edit commands in ONE response."
                    )
