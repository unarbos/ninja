...
MAX_NO_COMMAND_REPAIRS = 3  # 770-king: one extra repair beats early empty-patch return on transient model formatting drift
...
MAX_STEP_RETRIES = 2
WALL_CLOCK_BUDGET_SECONDS = 265.0  # slightly smaller limit for better safety
...
def _solve_attempt(**kwargs: Any) -> Dict[str, Any]:
    ...
    def maybe_queue_refinement(assistant_text: str) -> bool:
        ...
        # Correctness gates above (syntax + test) come BEFORE completeness
        # gates below (coverage + criteria) — fix what's broken before asking
        # the model to add more. This also matches the docstring at the top of
        # `maybe_queue_refinement` and the 770-king ordering.
