...
def _extract_issue_path_mentions(_issue_content: str) -> List[str]:
    pattern = re.compile(
    ...
    mentions: List[str] = []
    for match in pattern.finditer(_issue_content):
...
def _issue_terms(_issue_content: str) -> List[str]:
    stop = {
        "about",
        "after",
        "also",
    ...
    terms: List[str] = []
    for raw in re.findall(r"[A-Za-z_][A-Za-z0-9_-]{2,}", _issue_content.lower()):
