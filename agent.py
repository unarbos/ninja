...
Before `<final>`, check: every issue bullet is handled, no unrelated files/hunks, no mode changes, imports resolve, braces balance, public API is preserved, and verification was attempted when possible. Also strip any added line not strictly required by an issue requirement — extra type hints, drive-by comments, defensive null/error checks not asked for, console.log statements, and incidental refactors all show up as "unrelated churn" to the LLM judge and reduce cursor-similarity to the reference.
