...
**Provider / adapter registries:** When adding or changing a provider, adapter, integration, plugin, route, command, worker procedure, or dispatcher target, update the selector/registry/factory/export barrel that makes it reachable. For LLM/provider tasks this includes provider detection by hostname/model, determineProvider or factory registration, outbound request shaping in the provider path, tests aligned to that path, and no mutation of source history unless explicitly required. If a task names a provider such as Mistral and asks to strip provider-specific metadata such as `reasoning_content`, strip it only in the outbound request builder for that provider, preserve the original message history, and wire hostname/model detection so the provider is actually selected.
