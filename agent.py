...
MAX_STEP_RETRIES = 2
WALL_CLOCK_BUDGET_SECONDS = 450.0
WALL_CLOCK_RESERVE_SECONDS = 30.0
# Pre-emptive stub trips at 65% of budget (≈273 s on default 420). If the
# loop has produced no patch by then we drop in the always-diff stub before
...
def _static_sanity_one(repo: Path, relative_path: str) -> Optional[str]:
    ...
    if suffix == ".py":
        ast_err = _check_python_syntax_one(repo, relative_path)
        if ast_err:
            return ast_err
        return _check_python_extras(repo, relative_path)
    if suffix in {".js", ".mjs", ".cjs"}:
        result = _check_node_syntax_one(repo, relative_path)
        if result is not None:
            return result
        # node missing — fall back to brace balance.
        return _check_brace_balance_one(repo, relative_path, comment_style="c")
    if suffix in {".json"}:
        return _check_json_syntax_one(repo, relative_path)
    if suffix in {".yml", ".yaml"}:
        return _check_yaml_one(repo, relative_path)
    if suffix in {".toml"}:
        return _check_toml_one(repo, relative_path)
    if suffix in {".md", ".markdown", ".mdx"}:
        return _check_markdown_one(repo, relative_path)
    if suffix in {".html", ".htm", ".xhtml", ".vue", ".svelte"}:
        return _check_html_one(repo, relative_path)
    if suffix in _CSS_LIKE_SUFFIXES:
        return _check_brace_balance_one(repo, relative_path, comment_style="c")
    if suffix in _SHELL_SUFFIXES or name in ("makefile",):
        if name in ("makefile",) or suffix == ".mk":
            return _check_makefile_one(repo, relative_path)
        return _check_shell_one(repo, relative_path)
    if suffix in _C_LIKE_SUFFIXES:
        return _check_brace_balance_one(repo, relative_path, comment_style="c")
    # Everything else: trust the model; the LLM judge catches gross errors.
    return None
