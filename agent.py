...
# Empty-patch failure mode at 8192: the model rambles to the token cap on
# multi-step responses, individual calls eating 100+ seconds and consuming most
# of the inner attempt's wall-clock budget before any <command> block is
# emitted. 4096 forces tighter, command-emitting responses and keeps step
# latency under ~30s while still leaving headroom for plan + multi-edit batches.
...
def solve(
    ...
    # Empty-patch Mode 2 escape: attempt 1 produced no patch AND its log shows
    # repeated MODEL_ERROR markers. Re-running the same prompt chain through
    # _solve_attempt would likely hit the same proxy/transport failures and
    # waste the remaining budget. Switch to a single-call, low-token, hard
    # 45-second emergency prompt that bypasses the planning chain entirely.
    # If it produces any substantive edit, return it. If it also fails, fall
    # through to the standard same-prompt retry below.
    _attempt1_logs = _result1.get("logs", "") or ""
    _attempt1_was_error_failure = (
        not _patch1.strip()
        and _attempt1_logs.count("MODEL_ERROR") >= 2
    )
    if _attempt1_was_error_failure and (_multishot_total_budget - _elapsed) > 60.0:
...
def _solve_attempt(**kwargs: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        for step in range(1, max_steps + 1):
            ...
            # Adaptive token budget: when we are deep into the step loop with
            # no patch yet, the model is usually rambling rather than emitting
            # commands. Halve the per-call cap to push it toward terse,
            # command-first responses. Empty-patch traces show repeated 100s+
            # generations at the full cap that never reach a <command> block.
            adaptive_max_tokens = max_tokens
            if step >= 5 and not get_patch(repo).strip():
                adaptive_max_tokens = max(1024, max_tokens // 2)
