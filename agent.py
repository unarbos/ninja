...
def _extract_acceptance_criteria(issue_text: str) -> List[str]:
    ...
    for line in issue_text.splitlines():
        ...
        # Loosen length floor to 5 chars (was 6) — captures shorter bullet
        # criteria like "fix X.go" or "add Y" that we previously dropped.
        # Lifted from ProjectNobi's PR #274 — they got 46% per-round win
        # rate against the same king with this single-char tweak among
        # other small changes.
        if len(text) < 5:
            continue
