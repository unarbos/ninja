...
MAX_STEP_RETRIES = 2
WALL_CLOCK_BUDGET_SECONDS = 200.0  # further reduced for timeout safety (was 240)
...
## Functional completeness (applies to FEATURE_BUILD; skip for REFACTOR/BUG_FIX/MIGRATION)
...
def _build_v33_initial_user_message(repo: Path, issue_text: str, repo_summary: str, preloaded_context: str) -> str:
    ...
    if multi_file:
        strategy = (
            ...
            "Focus on correctness and completeness — do not add scope beyond what the task requires."
        )
    else:
        strategy = (
            ...
            "to working implementations. Keep changes minimal — only what the task explicitly requires."
        )
...
def _solve_attempt(**kwargs: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        _tle_emergency_threshold = max(WALL_CLOCK_BUDGET_SECONDS - 40.0, 60.0)  # 40s margin (was 60s) — more thinking time
