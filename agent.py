...
**First response rule**: in the SAME response as your first command, emit a short `<plan>` block listing each requirement and the target file/function for each. Then immediately issue the command. Do not send a plan-only response.
...
# v69 (j.py): 540s outer envelope leaves ~60s before typical 600s docker kill for safety net.
