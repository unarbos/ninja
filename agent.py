...
MAX_STEP_RETRIES = 2
# Default per-attempt wall clock. Overridden per call by `_estimate_wall_clock_budget`,
# which probes the live repo+issue with bash and runs the calibration from
# `_agents/time_limit_estimate.py` (least-squares fit against tasks 001-100). The
# constant below is the static fallback when the probe fails or the dynamic
# estimate is unavailable (it preserves prior 770_2.py behavior). Kept in
# [120, 600] per validator-style wall-clock band.
_WALL_CLOCK_FALLBACK_UNCLAMPED = 270.0
WALL_CLOCK_BUDGET_SECONDS = max(120.0, min(600.0, float(_WALL_CLOCK_FALLBACK_UNCLAMPED)))
WALL_CLOCK_RESERVE_SECONDS = 20.0

# --- Dynamic per-attempt wall clock (calibrated on tasks 001-100; bash probe) ---
# Calibration coefficients, blend weights, and ratio-target scale are copied
# verbatim from `_agents/time_limit_estimate.py`. They were fit to the dynamic
# ceiling `max(min(600, 2*cursor+1), 120)` over historical Cursor solve times on
# tasks 001-100, so the estimator returns a number in roughly the same regime
# as the validator's effective time ceiling. We clamp the result into a band
# that still leaves the multi-shot envelope (`_MULTISHOT_TOTAL_BUDGET`) room
# for a retry on small/medium tasks.
_CAP_BETA: Tuple[float, ...] = (
    -1502.280905,
    17.15374,
    24.155525,
    -25.281183,
...
_LIN_BLEND = 0.488
_STRUCT_BLEND = 1.06
_RATIO_TARGET_SCALE = 1.68
_DYNAMIC_BUDGET_MIN = 120.0
_DYNAMIC_BUDGET_MAX = 600.0
_TIME_PROBE_TIMEOUT_SECONDS = 20

# Single bash session: walks the running project tree (cwd is the repo),
# mirrors `_agents/time_limit_estimate.py` `_PROBE_BASH` tree metrics plus
# optional `reference.patch` stats (same wc/grep shape as the calibration
# script). Prompt/task size is not in this TSV — the validator `issue` string
# supplies `prompt_bytes` in Python (runtime analog of `task.txt`).
_TIME_PROBE_BASH = r"""set -euo pipefail
...
PATCH=""
for cand in task/reference.patch reference.patch; do
  if [ -f "$cand" ]; then PATCH="$cand"; break; fi
done
if [ -n "$PATCH" ]; then
  patch_b=$(wc -c <"$PATCH" 2>/dev/null | tr -d "[:space:]" || echo 0)
  case "$patch_b" in ''|*[!0-9]*) patch_b=0 ;; esac
  patch_lines=$(wc -l <"$PATCH" 2>/dev/null | tr -d "[:space:]" || echo 0)
  case "$patch_lines" in ''|*[!0-9]*) patch_lines=0 ;; esac
  diff_git=$(grep -c '^diff --git ' "$PATCH" 2>/dev/null | tr -d "[:space:]" || echo 0)
  case "$diff_git" in ''|*[!0-9]*) diff_git=0 ;; esac
else
  patch_b=0
  patch_lines=0
  diff_git=0
fi
printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$files" "$dirs" "$code" "$lines" "$patch_b" "$patch_lines" "$diff_git"
"""
...
# -----------------------------
# Dynamic wall-clock estimator
# -----------------------------
#
# These helpers port the time-limit estimator from
# `_agents/time_limit_estimate.py` (only the guessing logic; the reporting /
# logging code is intentionally left out). The probe runs through the same
# bash-via-subprocess style used everywhere else in this file
# (`shell=True, executable="/bin/bash"`), with `cwd` set to the running repo.
#
# `_estimate_wall_clock_budget` is the only entry point used elsewhere — it
# returns a number of seconds in ``[_DYNAMIC_BUDGET_MIN, _DYNAMIC_BUDGET_MAX]``
# (120–600), computed from the project's filesystem shape, optional
# ``reference.patch`` metrics from the bash probe, plus the byte length of the
# ``issue`` text passed by the validator (the runtime analog of ``task.txt``).

def _time_probe_feature_vector(probe: Tuple[int, int, int, int, int, int, int, int]) -> List[float]:
...
def _linear_cap_hat(features: List[float]) -> float:
    pred = sum(b * x for b, x in zip(_CAP_BETA, features))
    return float(max(120.0, min(600.0, pred)))


def _structural_score(probe: Tuple[int, int, int, int, int, int, int, int]) -> float:
    """Repo-only complexity score; mirrors `_structural_score` in time_limit_estimate.py."""
...
def _estimate_seconds_from_probe(probe: Tuple[int, int, int, int, int, int, int, int]) -> int:
    feats = _time_probe_feature_vector(probe)
    cap_hat = _linear_cap_hat(feats)
    struct = _structural_score(probe)
    blended = min(_LIN_BLEND * cap_hat, _STRUCT_BLEND * struct)
    scaled = blended * _RATIO_TARGET_SCALE
    est = int(round(scaled))
    return max(30, min(est, 600))


def _run_time_probe(repo: Path) -> Tuple[int, int, int, int, int, int, int]:
    """Run `_TIME_PROBE_BASH` against `repo` and parse probe columns.

    Returns ``(files, dirs, code_files, lines, patch_bytes, patch_lines,
    diff_git_count)`` — the same patch-shape fields as
    ``_agents/time_limit_estimate.py`` when ``task/reference.patch`` or
    ``./reference.patch`` exists; otherwise zeros.

    Uses 770_2.py's standard bash-invocation style. Any failure (non-zero exit,
    timeout, malformed output) collapses to all zeros so the estimator can still
    degrade gracefully to its prompt/structural arms.
    """
    try:
        proc = subprocess.run(
            _TIME_PROBE_BASH,
            ...
            stderr=subprocess.PIPE,
            text=True,
            timeout=_TIME_PROBE_TIMEOUT_SECONDS,
            env=_command_env(),
        )
    except Exception:
    ...
    try:
        if len(parts) == 7:
            f, d, c, ln, pb, pl, dg = (int(x) for x in parts)
            return (f, d, c, ln, pb, pl, dg)
        if len(parts) == 4:
            f, d, c, ln = (int(x) for x in parts)
            return (f, d, c, ln, 0, 0, 0)
    except ValueError:
        pass
    return (0, 0, 0, 0, 0, 0, 0)


def _estimate_wall_clock_budget(repo_path: str | Path, issue: str) -> float:
    """Dynamic per-attempt wall clock budget (seconds) for ``repo_path`` + ``issue``.

    The probe captures the project's filesystem shape and, when present,
    ``task/reference.patch`` or ``reference.patch`` (same layout as the
    calibration harness) supplies patch-byte / hunk-count features matching
    ``_agents/time_limit_estimate.py``. The validator ``issue`` string's UTF-8
    length stands in for ``task.txt`` prompt bytes. Any probe or estimate
    failure returns ``WALL_CLOCK_BUDGET_SECONDS`` (clamped to [120, 600]).
    """
    try:
        repo = Path(repo_path).resolve()
    except Exception:
        return float(WALL_CLOCK_BUDGET_SECONDS)
    if not repo.is_dir():
        return float(WALL_CLOCK_BUDGET_SECONDS)

    files, dirs, code, lines, patch_b, patch_lines, diff_git = _run_time_probe(repo)
    try:
        prompt_bytes = len((issue or "").encode("utf-8"))
    except Exception:
        prompt_bytes = len(issue or "")
    probe = (files, dirs, code, lines, prompt_bytes, patch_b, patch_lines, diff_git)
    try:
        seconds = _estimate_seconds_from_probe(probe)
    except Exception:
        return float(WALL_CLOCK_BUDGET_SECONDS)
    return float(max(_DYNAMIC_BUDGET_MIN, min(_DYNAMIC_BUDGET_MAX, seconds)))
...
def solve(
    ...
    Dynamic wall clock: instead of the static 270s budget, we probe the
    running repo + issue text and call the calibration ported from
    `_agents/time_limit_estimate.py` to pick a per-attempt budget tuned to
    the project's complexity. Falls back silently to `WALL_CLOCK_BUDGET_SECONDS`
...
def _solve_with_safety_net(**kwargs: Any) -> Dict[str, Any]:
    ...
    1. There is intentionally no third "emergency" single-shot fallback.
       The per-attempt budget (now dynamic via `_estimate_wall_clock_budget`,
       formerly the static `WALL_CLOCK_BUDGET_SECONDS=270s`) is sized so two
       attempts can comfortably fit inside `_MULTISHOT_TOTAL_BUDGET=580s` for
       small/medium tasks, while complex tasks consume one larger attempt and
       skip retry on the `_MULTISHOT_MIN_ATTEMPT_RESERVE` check below. A third
       attempt would have to be either (a) so short it cannot produce a patch,
       or (b) push past the validator's per-round soft cap and forfeit the
       round entirely. The empty-patch case is already handled inside
       `_solve_attempt` by the hail-mary refinement turn (see
       `maybe_queue_refinement`), and any uncaught exception is caught by the
       `except Exception` arm below which returns the on-disk patch verbatim.
...
def _solve_attempt(**kwargs: Any) -> Dict[str, Any]:
    ...
    # Per-attempt wall clock. `solve()` computes this from a bash probe over
    # the live repo + issue text via `_estimate_wall_clock_budget`. When this
    # function is called directly (tests, ad-hoc), fall back to the static
    # `WALL_CLOCK_BUDGET_SECONDS` so prior behavior is preserved.
    wall_clock_budget = float(kwargs.get("wall_clock_budget") or WALL_CLOCK_BUDGET_SECONDS)
    ...
    # return whatever patch is already on disk. `wall_clock_budget` itself
    # is dynamic — see `_estimate_wall_clock_budget` for the calibration.
