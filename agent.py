...
def _unaddressed_criteria(patch: str, issue_text: str) -> List[str]:
    """Acceptance criteria whose keywords don't show up in the added lines.

    A criterion counts as "addressed" when at least `threshold` of its content
    keywords appear in the patch's added text (including stem-matching for
    dotted path keywords). Threshold scales with the criterion's keyword
    count: short criteria require one match, long ones require two.

    Returns the criteria most likely missed; capped at _MAX_CRITERIA.
    """
    ...
    for crit in criteria:
        ...
        if not keywords:
            continue
        # 1-3 keywords: 1 hit; 4-5: 1 hit; 6+: 2 hits.
