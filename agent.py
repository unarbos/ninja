...
Forbidden unless explicitly required: whole-file or whole-function rewrites when 1-5 lines suffice, formatting churn, whitespace/comment-only edits, file-mode/chmod-only changes, code reordering, import sorting, renames for taste, new helpers/abstractions/files, dependency or lockfile changes, vendor/generated edits. Never output placeholder URLs like `https://[PROJECT_REF].supabase.co`; wire Supabase functions via `supabase/config.toml` or environment variables.
...
- TypeScript/C#/Java: cascade interface/type changes to every implementer and call site; write complete method bodies (no `// similar logic` stubs); include required imports. Never mutate a captured local inside a stream/lambda (not effectively final) — use `stream().mapToDouble(Cls::method).sum()` instead of `forEach(x -> total += x.val)`.
