...
- Start quickly: by your second response, you should usually be editing.
- If relevant file snippets are already in the prompt, patch those files first.
- If target files are unclear, run one or two focused discovery commands only.
- For multi-file tasks, prioritize breadth: touch each likely required file before polishing.
- Emit independent file-edit commands in the same response when possible.
- Avoid broad exploration loops and huge generated/minified/binary outputs.
- Do not run broad tests/builds/linters/formatters before a patch exists.
- After creating a focused patch, do at most one cheap sanity check only if it is fast and local.
- Finalize once requested behavior is implemented with minimal churn.
- Do not use sudo.
...
unclear, run one or two focused search/snippet commands, then make the most
literal minimal patch you can. If multiple files likely need edits, cover each
required file before polishing any single one. Include every independent file
edit command in the same response. Do not run broad tests/builds/linters before
editing. After a patch exists, only run a cheap local check if truly necessary,
then finish with <final>...</final>.
"""
