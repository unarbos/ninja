...
def _sanitize_patch(diff_output: str) -> str:
    ...
    cleaned = _strip_garbage_named_file_diffs(diff_output)
...
# Filenames git can produce that look like shell-corruption artifacts.
# In duel 004723 round 064608 king lost margin 0.537 because its agent ran a
# malformed `python3 -c` whose unescaped braces created a real file named
# `1 in code}')\nprint(f'has sm=6 conditional: {sm={6 in code or sm={`.
# git diff then emitted a `new file mode 100644` block for that filename and
# the LLM judge marked the entire patch as "unrelated garbage churn".
# Real source files never contain shell-metadata, brace expansions, or
# unescaped newlines in their path — discard such blocks defensively.
_GARBAGE_FILENAME_RE = re.compile(r"""[\n\r\t\x00-\x1f`${}\\]|\\n|\\r|\\t""")
_GARBAGE_FILENAME_TOKENS = ("')", "f'", "f\"", "print(", "}')", "={", "});")


def _looks_like_garbage_filename(path: str) -> bool:
    if not path:
        return False
    # git wraps special-character paths in double quotes — most legitimate
    # source paths do not.  Quoting alone is not a smoking gun (think
    # "spaces in filename.txt"), so layer two more checks:
    if _GARBAGE_FILENAME_RE.search(path):
        return True
    if any(tok in path for tok in _GARBAGE_FILENAME_TOKENS):
        return True
    return False


def _strip_garbage_named_file_diffs(diff_output: str) -> str:
    """Discard diff blocks whose filename looks like shell-corruption output.

    Triggers only on filenames with control characters, shell metachars, or
    fragments of Python/JS source code (f-string interpolation, print(), …).
    """
    if not diff_output.strip():
        return diff_output
    blocks = re.split(r"(?=^diff --git )", diff_output, flags=re.MULTILINE)
    ...
    for block in blocks:
        if not block:
            continue
        first = block.splitlines()[0] if block else ""
        # Accept both unquoted (a/path b/path) and quoted ("a/path" "b/path")
        # forms.  Quoted form is what git uses for special-character paths.
        m = re.match(r'diff --git "?a/(.+?)"? "?b/(.+?)"?$', first)
        if m:
            a_path, b_path = m.group(1), m.group(2)
            if _looks_like_garbage_filename(a_path) or _looks_like_garbage_filename(b_path):
                continue
        kept.append(block)
    result = "".join(kept)
    if diff_output.endswith("\n") and result and not result.endswith("\n"):
...
# Comment-glued-import pattern: in duel 004723 round 065045, king's agent
# edited a JSX file and accidentally produced:
#     // Step 5-4 — comment textimport { useState, useMemo } from "react";
# Two source lines collapsed onto one. The judge flagged the file as a
# critical regression because `useState` is now inside a comment and never
# imported, so the entire component fails to compile. Same pattern shows up
# in Python with `# comment textfrom x import y`.
_COMMENT_GLUED_JS_RE = re.compile(
    r"""^\s*(?://|/\*)[^\n\r]*?\S(import\s*(?:\{|\*|[A-Za-z_$]))""",
    re.MULTILINE,
)
_COMMENT_GLUED_PY_RE = re.compile(
    r"""^\s*\#[^\n\r]*?\S((?:from\s+\S+\s+import|import)\s+[A-Za-z_])""",
    re.MULTILINE,
)


def _check_comment_glued_imports(repo: Path, relative_path: str) -> List[str]:
    """Flag files where a comment line is glued to an `import` statement.

    The agent's sed-style edits sometimes drop the newline between a comment
    being kept and an import being kept, producing a single line that looks
    like a comment but contains the import keyword inside it. The compiler
    treats the import as comment text and downstream code fails because the
    expected names are never bound.
    """
    full = (repo / relative_path).resolve()
    try:
        full.relative_to(repo.resolve())
    ...
    suffix = Path(relative_path).suffix.lower()
    if suffix in _COMPILE_SANITY_JS_LIKE:
        pat = _COMMENT_GLUED_JS_RE
    elif suffix == ".py":
        pat = _COMMENT_GLUED_PY_RE
    else:
        return []
    ...
    for m in pat.finditer(source):
        line_no = source.count("\n", 0, m.start()) + 1
        snippet = source[m.start(): m.start() + 90].split("\n", 1)[0]
        findings.append(
            f"{relative_path}:{line_no}: comment line is glued to an import "
            f"statement — split onto separate lines. Saw: {snippet!r}"
        )
        if len(findings) >= 3:
            break
    return findings
...
def _check_compile_sanity(repo: Path, patch: str) -> List[str]:
    ...
    Targets patterns the parser/node check cannot catch but that consistently
    broke king's diffs across duels 4696-4743:
      ...
      - Comment lines glued to import statements (no newline separator)
    """
    errors: List[str] = []
    ...
    for relative_path in _patch_changed_files(patch):
        ...
        if suffix == ".dart":
            ...
            errors.extend(_check_comment_glued_imports(repo, relative_path))
        elif suffix == ".py":
            errors.extend(_check_comment_glued_imports(repo, relative_path))
        if len(errors) >= 6:
            break
...
- When you lift state up, declare BOTH the value and its setter at the new parent (e.g. `const [chainId, setChainId] = useState(...)`) AND pass them down — partial lifts are a common silent break.
- After every `sed`/`python -c` edit that removed or rewrote lines near `import`, `from`, or top-of-file comments, `cat -n` the first 30 lines of the touched file and verify NO comment line and no import statement are glued onto the same physical line (a `// ... import {{` or `# ... from x import` joined together kills the build and the LLM judge will flag a critical regression).
Silent compile breaks are the most common loss mode.

Style alignment with reference patches (high-leverage on the LLM judge):
- When a JSX/Vue/Angular template repeats the SAME boolean expression 3 or more times (role checks, feature flags, permission gates), extract it to a single `const canX = ...` above the `return` and reuse the name. Inlining the expression three times — even if correct — costs alignment points the reference always claims.
- Prefer `useMemo`/`computed` over an immediately-invoked function expression `(() => { ... })()` for derived values inside JSX/template.
- Do NOT introduce new files or wholesale rewrites of files the task did not name. Add a helper to an EXISTING module before creating a new module.
...
def build_compile_sanity_fix_prompt(errors: List[str]) -> str:
    ...
    All three finding classes (Dart Consumer-base mismatch, broken relative
    import path, comment-glued-onto-import) are guaranteed build failures in
    their target language. The fix is mechanical, so the prompt nails down
    ...
    return (
        ...
        "  - Comment-line glued to import statement (e.g. `// commentimport { useState }` or "
        "`# commentfrom x import y`) -> split the comment and the import onto SEPARATE physical "
        "lines. Use a python one-liner that reads the file, replaces the merged line with "
        "`<comment_text>\\n<import_text>`, and writes it back — sed quoting is fragile here.\n\n"
