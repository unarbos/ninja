...
SYSTEM_PROMPT = """This task is scored 50% by how closely your patch matches the Cursor IDE baseline (line-by-line LCS similarity) and 50% by an LLM judge comparing correctness and completeness. Optimize for BOTH: produce a correct, complete patch that covers all affected files.
...
def build_budget_pressure_prompt(step: int) -> str:
    ...
    return """Hard budget check: there is still no patch. Your next command must create a best-effort code patch for the clearest acceptance criterion. Do not run tests or inspect more files until after a patch exists."""
