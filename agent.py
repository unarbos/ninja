...
def build_preloaded_context(repo: Path, issue: str) -> Tuple[str, List[str]]:
    ...
    try:
        ...
        _criteria_block = _format_criteria_checklist(_criteria)
    except Exception:
        _criteria_block = ""
    if _criteria_block:
        parts.insert(0, _criteria_block)
...
def _rank_context_files(repo: Path, issue: str) -> Tuple[List[str], int]:
    ...
    # Quoted-evidence path boost.
    try:
        _quoted_evidence_boost = _quoted_evidence_paths(repo, tracked_set, issue)
    except Exception:
        _quoted_evidence_boost = {}
    ...
    for relative_path in tracked:
        ...
        score += _quoted_evidence_boost.get(relative_path, 0)
        if score > 0:
...
def _check_syntax(repo: Path, patch: str) -> List[str]:
    ...
    try:
        _added_names = _patch_added_names_set(patch)
    except Exception:
        _added_names = set()
    for relative_path in _patch_changed_files(patch):
        ...
        if suffix == ".py":
            ...
            result = _check_brace_balance_one(repo, relative_path)
        # v51: kept TOML/Go/Rust extended syntax; stripped TS tsc (too slow)
        elif suffix == ".toml":
        ...
        # v47: corruption + duplicate-imports advisory checks (port from pr733)
        if result is None:
            try:
                _corruption_finding = _check_corruption_one(repo, relative_path)
            except Exception:
                _corruption_finding = None
            if _corruption_finding:
                errors.append(_corruption_finding)
            try:
                _duplicate_import_finding = _check_duplicate_imports_one(repo, relative_path)
            except Exception:
                _duplicate_import_finding = None
            if _duplicate_import_finding:
                errors.append(_duplicate_import_finding)
        # v44: test-quality gate on touched test files when patch added names
        if result is None and _is_test_path(relative_path) and _added_names:
            try:
                tq = _check_test_quality_one(repo, relative_path, _added_names)
            except Exception:
                tq = None
            if tq:
                errors.append(tq)
    # v51: stripped contract preservation git-grep (heavy subprocess overhead)
    return errors
...
# wall budget.
_PREFLIGHT_TEST_TOTAL_BUDGET_SECONDS = 10
_PREFLIGHT_TEST_PER_TARGET_TIMEOUT = 6
...
def _multishot_quality(
    ...
    broad = (
        issue_text.count("\n- ") >= 6
        or len(_extract_issue_path_mentions(issue_text)) >= 5
    )
    if broad:
...
_CRITERIA_LEAD_RE = re.compile(
    r"(?:^|\n)\s*(?:[-*•]|\d+\.|\d+\))\s+([^\n]{8,200})"
)
_CRITERIA_CONJUNCTION_RE = re.compile(
    r"\b(?:and|also|must|should|when|unless|only|both|ensure|preserve)\b",
    re.IGNORECASE,
)
_CRITERIA_MAX_COUNT = 8
_CRITERIA_MIN_LEN = 10
_CRITERIA_MAX_LEN = 200


def _extract_acceptance_criteria(issue: str) -> List[str]:
    """Extract criterion-like clauses from issue text. Bullets + conjunctions."""
    if not issue or not issue.strip():
        return []
    found: List[str] = []
    seen: set = set()
    for m in _CRITERIA_LEAD_RE.finditer(issue):
        text = m.group(1).strip()
        text = re.sub(r"\s+", " ", text)
        if len(text) < _CRITERIA_MIN_LEN or len(text) > _CRITERIA_MAX_LEN:
            continue
        key = text.lower()
        if key in seen:
            continue
        seen.add(key)
        found.append(text)
        if len(found) >= _CRITERIA_MAX_COUNT:
            return found
    if len(found) < 3:
        for sentence in re.split(r"(?<=[.!?])\s+", issue):
            sentence = sentence.strip()
            if (len(sentence) < _CRITERIA_MIN_LEN
                or len(sentence) > _CRITERIA_MAX_LEN):
                continue
            if not _CRITERIA_CONJUNCTION_RE.search(sentence):
                continue
            key = sentence.lower()
            if key in seen:
                continue
...
def _format_criteria_checklist(criteria: List[str]) -> str:
    if not criteria:
        return ""
...
# Constants used by the quoted-evidence path ranker.
...
_CORRUPTION_SKIP_SUFFIXES = {".sh", ".bash", ".zsh", ".fish", ".md", ".txt", ".rst", ".zig", ".rs", ".go", ".cs", ".kt"}
_TS_SYNTAX_ERROR_RE = re.compile(r"\berror\s+(TS\d{4,5})\b")

_CONTRACT_GREP_TIMEOUT_SECONDS = 4
_CONTRACT_MAX_FINDINGS = 4
_CONTRACT_NAME_DENYLIST = {
    "self", "cls", "this", "default", "Object", "Function", "True", "False",
    "None", "null", "undefined", "var", "let", "const", "type",
}
_REMOVED_PUBLIC_SYMBOL_PATTERNS = (
    re.compile(
...
def _emergency_pick_target(repo: Path, task_text: str) -> Optional[str]:
    ...
    for mention in mentioned_paths:
        ...
        if normalized in tracked and _context_file_allowed(normalized):
            return normalized
    # _rank_context_files may return either a list or (list, top_score).
    _rank_result = _rank_context_files(repo, task_text)
    if isinstance(_rank_result, tuple):
        ranked = _rank_result[0]
    else:
        ranked = _rank_result
...
def _solve_emergency_single_shot(**kwargs: Any) -> Dict[str, Any]:
    """Single-call fallback for empty-patch and MODEL_ERROR runs.
    Used as a last-resort step when the multishot pipeline returns empty."""
...
def _solve_with_safety_net(**kwargs: Any) -> Dict[str, Any]:
    def _maybe_emergency_fallback(result: Dict[str, Any], started: float) -> Dict[str, Any]:
        """If result has no patch but we still have budget, fire emergency single-shot."""
        try:
            ...
            if (_MULTISHOT_TOTAL_BUDGET - elapsed) < 100:
                return result
            ...
            if emer_patch.strip():
                ...
                merged["v50_emergency_invoked"] = True
                return merged
        except Exception:
            pass
        return result
    ...
    try:
        ...
        return _maybe_emergency_fallback(_result1, _multishot_started)
