...
MAX_MISSING_IMPORT_TURNS = 1    # v7: AST-detect Python names used without imports/defs; fix before final
...
# -----------------------------
# v7 — AST-based missing-import detector (Python only)
# -----------------------------
#
# Catches NameError-causing patches at static-analysis time. Common case
# from real validator rationales: agent edits a Python file, references a
# function name, but forgets to add the corresponding import. The result
# is a compile-clean patch that fails the moment it runs, scoring ~0 on
# the LLM judge.
#
# Edge A's identifier-verify gate looks at identifiers ADDED by the patch
# and flags those not seen elsewhere in the repo. This gate is different:
# it AST-walks the FULL post-patch file, builds the set of names defined
# (imports, def, class, assignments, function args, comprehension targets,
# with-as bindings, etc.), the set of names USED (Name nodes in Load
# context), and reports `used - defined - builtins` per file.

_AST_PSEUDO_NAMES = frozenset({
    "self", "cls",
    "__name__", "__file__", "__doc__", "__package__", "__loader__",
    "__spec__", "__builtins__", "__class__", "__dict__", "__annotations__",
})


def _detect_missing_imports_py(repo: Path, file_path: str, max_results: int = 8) -> List[str]:
    """AST-detect Python identifiers used but not imported or defined.

    Returns at most `max_results` candidate missing names from the post-patch
    file. Returns empty list on syntax error (let _check_syntax handle that),
    on star imports (can't analyze), or on file read error.
    """
    try:
        import ast as _ast
        ...
        for node in _ast.walk(tree):
            if isinstance(node, _ast.Import):
                ...
                for arg in (
                    ...
                    + list(getattr(node, "posonlyargs", []) or [])
                ):
                    defined.add(arg.arg)
                if node.vararg:
        ...
        try:
            ...
            builtin_names = set()
        builtin_names |= _AST_PSEUDO_NAMES
        # Skip dunder + leading-underscore names (likely internal / builtins).
        missing = sorted(
            name for name in (used - defined - builtin_names)
            if not name.startswith("_")
        )
        return missing[:max_results]
    except Exception:
        return []


def _patch_missing_imports(patch: str, repo: Path) -> List[Tuple[str, List[str]]]:
    """Per-file missing-import scan across files touched by the patch (Python only)."""
    out: List[Tuple[str, List[str]]] = []
    for fp in _patch_changed_files(patch):
        if fp.endswith(".py"):
...
def build_missing_imports_prompt(missing: List[Tuple[str, List[str]]]) -> str:
    """Tell the model which Python names are USED in the patched file but
    have no import or definition. Catches NameError-causing patches before
    final.

    Real validator rationales repeatedly cite missing imports as a loss
    reason. The fix is usually one line per name (add `from X import Y`).
    """
    sections: List[str] = []
    for fp, names in missing[:6]:
        bullets = ", ".join(f"`{n}`" for n in names[:6])
        sections.append(f"- {fp}: {bullets}")
    bullets = "\n  ".join(sections) or "(none)"
    return (
        "Missing-import check (Python AST analysis) — these names are USED in "
        "your post-patch file(s) but are NEITHER imported NOR defined locally:\n"
        f"  {bullets}\n\n"
        "Each one will raise NameError at runtime, scoring 0 on the LLM judge.\n\n"
        "For each name, either:\n"
        "  (a) add the missing import (e.g. `from os.path import join` at the top "
        "of the file, alongside existing imports), OR\n"
        "  (b) the name is a typo of an existing local — rename the use site to "
        "the real name, OR\n"
        "  (c) the name is meant to be a parameter / loop variable / new helper "
        "you forgot to define — add the definition.\n\n"
        "Run `head -30 <file>` to see the existing import block, then issue "
        "sed -i / python -c command(s) to fix the imports. Then end with "
        "<final>summary</final>."
    )
...
def solve(
    ...
    missing_import_turns_used = 0     # v7: AST-detected NameError fix
    ...
    def maybe_queue_refinement(assistant_text: str) -> bool:
        ...
        """
        nonlocal polish_turns_used, self_check_turns_used, syntax_fix_turns_used, coverage_nudges_used, criteria_nudges_used, identifier_verify_turns_used, missing_import_turns_used, hail_mary_turns_used
        ...
        # v7 — missing-import AST gate. Fires BEFORE coverage/criteria/
        # identifier-verify because a NameError-causing patch tanks the
        # judge's correctness score regardless of coverage. Python only;
        # other languages have no cheap equivalent. Skip when wall-clock is
        # tight — the AST walk is fast but the model's fix turn isn't.
        if (
            missing_import_turns_used < MAX_MISSING_IMPORT_TURNS
            ...
            if missing_imports:
                ...
                summary = ", ".join(
                    f"{fp}({len(names)})" for fp, names in missing_imports[:4]
                )
                queue_refinement_turn(
                    assistant_text,
