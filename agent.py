...
# observation size, and max_tokens. Sampling parameters are validator-owned
# and overwritten server-side; do not set them.
...
MAX_SELF_CHECK_TURNS = 0   # disabled: hurts simple tasks more than it helps
MAX_SYNTAX_FIX_TURNS = 1   # repair Python/TypeScript/JavaScript SyntaxError
MAX_TEST_FIX_TURNS = 1     # repair the companion test we ran ourselves
MAX_PATCH_BLOCK_BYTES = 64000  # cap on a single <patch> block; rejects pathological inputs
MAX_HEREDOC_FAIL_STREAK = 2  # heredoc failures before forcing alternative edit form
...
# Detects bash commands that DISCARD the agent's own work-in-progress edits.
# Firing this triggers a corrective prompt asking the model to re-apply the
# edit forward instead of throwing it away.
_DESTRUCTIVE_RE = re.compile(
    r"(^|[\s;&|`])"
    r"(git\s+(checkout(\s+--?\w+)*\s+--|restore\b|reset\s+--hard\b|clean\s+-[fdx]+|stash(\s+(push|pop|drop))?\b)"
    r"|rm\s+-[rRf]+\s+\S+)"
)

# Detects bash heredoc blocks; when these fail to apply repeatedly we steer the
# model toward a more deterministic edit form.
...
def apply_unified_diff(diff_text: str, repo: Path) -> PatchApplyResult:
    ...
    Strategies tried in order:
      ...
      2. `git apply --whitespace=fix --3way` — falls back when context lines drift
      3. `git apply --whitespace=fix -p0`    — handles diffs without a/ b/ prefix

    The diff body is written to a tempfile OUTSIDE the working tree to avoid
    leaking a stray `.pilot_pending.diff` into the patch output if anything
    fails mid-way.
    """
    ...
    try:
        with tempfile.NamedTemporaryFile(
            mode="w", suffix=".diff", prefix="agent_apply_", dir=tmp_dir, delete=False, encoding="utf-8"
        ) as fh:
            fh.write(body)
...
def _run_test_partner(repo: Path, test_path: str, timeout: int = 8) -> Optional[str]:
    """Run a single companion test file with the cheapest runner available.

    Returns the failure tail (≤2400 chars) if the test fails or errors,
    otherwise None. Silently returns None if the runner is missing,
    times out, or the file extension is not supported.
    """
    ...
    if suffix == ".py":
        ...
        # Prefer jest/vitest if available; otherwise skip — node --check is for syntax.
...
def _choose_test_partner_to_check(repo: Path, patch: str) -> Optional[Tuple[str, str]]:
    """Pick the first patch-touched source file with a discoverable test partner
    and run that test. Returns (test_path, failure_tail) on test failure, else None.
    """
...
- The preload includes companion test files alongside their source partners
  whenever both exist. When you patch a source file, update the companion
  test in the SAME <patch> if it is affected.
...
- When several files need changes, emit every independent file-edit command
  in the SAME response (or include them all in ONE <patch> block).
...
Reliability when using <command> for edits:
- After each bash edit, confirm it landed by running `git diff --stat <file>`;
  if the diff is empty, re-read the file and retry with the exact byte-for-byte
  original snippet (or switch to <patch>).
- For bash edits, prefer `python3 -c "..."` with an `assert OLD in s` guard so
  a missed pattern raises instead of silently no-opping; `sed -i` is fine for
  trivial single-token swaps; reserve heredocs for rewriting short whole files
  and use a unique delimiter.
- Do not discard your own work: avoid `git checkout -- <path>`, `git restore`,
  `git reset --hard`, `git clean`, or `git stash`. If an edit is wrong, fix
  it forward with another targeted edit (or a corrective <patch>).
...
def build_test_fix_prompt(test_path: str, output: str) -> str:
    ...
    return (
        ...
        "test needs to be updated to match the new behavior. Emit a corrective "
        "<patch> block (preferred) or minimal <command> blocks, then end with "
        "<final>summary</final>."
    )


def build_destructive_revert_repair_prompt(command: str) -> str:
    """Sent after the model runs a command that throws away its own work."""
    snippet = command.strip().splitlines()[0][:120]
    return (
        f"Notice: the previous command (`{snippet}`) discards repository changes "
        "(checkout/restore/reset --hard/clean/stash). Any earlier edits in this "
        "session may now be gone. Do NOT issue another revert. Instead, re-apply "
        "your intended changes forward as a focused <patch> block (or targeted "
        "edit commands), then end with <final>summary</final>."
    )


def build_repeated_heredoc_failure_prompt() -> str:
    """Sent after >=2 consecutive heredoc edits failed to land changes."""
    return (
        "Two heredoc edits in a row did not produce a diff. Heredocs are fragile "
        "with embedded quotes/indentation. Switch to a <patch> block (unified "
        "diff applied via git apply), or use `python3 -c \"...; assert OLD in s; "
        "Path(p).write_text(s.replace(OLD, NEW))\"` so a missed pattern raises "
        "loudly instead of silently no-opping. Then continue."
    )
...
def _command_uses_heredoc(command: str) -> bool:
    if not command:
        return False
...
def solve(
    ...
    try:
        ...
        for step in range(1, max_steps + 1):
            ...
            # Process direct unified-diff <patch> blocks before bash commands.
            # Each block is applied via `git apply` (whitespace fix + 3way fallback).
            # Outcomes are appended to observations so the model sees whether
            # its diff applied cleanly without a separate verification step.
            if patches:
                consecutive_no_command = 0
            ...
            destructive_repair_pending: Optional[str] = None
            heredoc_repair_pending = False
            patch_before_batch = get_patch(repo)

            for command_index, command in enumerate(command_batch, 1):
                if _is_destructive_command(command) and destructive_repair_pending is None:
                    destructive_repair_pending = command
                heredoc_used = _command_uses_heredoc(command)
                patch_before_cmd = get_patch(repo)
                ...
                if heredoc_used:
                    if get_patch(repo) == patch_before_cmd:
                        heredoc_fail_streak += 1
                        if heredoc_fail_streak >= MAX_HEREDOC_FAIL_STREAK:
                            heredoc_repair_pending = True
                            heredoc_fail_streak = 0
                    else:
                        heredoc_fail_streak = 0
            ...
            if observations:
                observation_text = "\n\n".join(observations)
                if destructive_repair_pending and not success:
                    observation_text += "\n\n" + build_destructive_revert_repair_prompt(
                        destructive_repair_pending
                    )
                    logs.append(f"\nDESTRUCTIVE_REPAIR_QUEUED: {destructive_repair_pending.strip().splitlines()[0][:80]}")
                elif heredoc_repair_pending and not success:
                    observation_text += "\n\n" + build_repeated_heredoc_failure_prompt()
                    logs.append("\nHEREDOC_REPAIR_QUEUED")
