...
# validator proxy owns sampling params and overwrites them server-side.
...
def _sanitize_output(text: str) -> str:
    """Strip control characters and truncate extremely long lines."""
    if not text:
        return text
...
def _escape_output_tags(text: str) -> str:
    """Prevent tool output from being parsed as agent action tags."""
...
def format_observation(result: CommandResult) -> str:
    ...
    if result.timed_out:
        parts.append(f"\nTIMED_OUT: true (after {result.duration_sec:.1f}s)")
    if result.blocked:
        parts.append("\nBLOCKED: true (dangerous command)")
...
_MARKDOWN_BARE_RE = re.compile(r"```\s*\n(.*?)```", re.DOTALL)
...
def _reset_file_modes(repo: Path) -> None:
    """Revert mode-only changes (chmod) that have no content diff."""
    try:
        proc = subprocess.run(
        ...
        for fname in proc.stdout.strip().splitlines():
            ...
            diff_text = show.stdout or ""
            has_mode = "\nold mode " in diff_text and "\nnew mode " in diff_text
            has_content = "\n@@ " in diff_text
...
def _strip_mode_only_file_diffs(diff_output: str) -> str:
    ...
    for block in blocks:
        ...
        if mode_only:
            continue
        cleaned_block = re.sub(r"^old mode \d+\n", "", block, flags=re.MULTILINE)
        cleaned_block = re.sub(r"^new mode \d+\n", "", cleaned_block, flags=re.MULTILINE)
...
def _hunk_is_import_reorder_only(added: List[str], removed: List[str]) -> bool:
    """Detect hunks that only reorder import statements."""
    if not added or not removed:
        return False
    a_stripped = sorted(s.strip() for s in added if s.strip())
    r_stripped = sorted(s.strip() for s in removed if s.strip())
    if a_stripped != r_stripped:
        return False
    import_kw = ("import ", "from ", "require(", "#include", "using ")
    return all(
        any(line.strip().startswith(kw) for kw in import_kw)
        for line in added + removed if line.strip()
    )
    ...
    for block in blocks:
        ...
        for hunk_text in hunks:
            ...
            if (
                ...
                or _hunk_is_import_reorder_only(added, removed)
            ):
                continue
        ...
        for step in range(1, max_steps + 1):
            ...
            if observations:
                ...
                if not success and get_patch(repo).strip():
                    missing = _uncovered_required_paths(get_patch(repo), issue)
                    if missing:
                        observation_text += (
                            f"\n\nPatch exists but does NOT touch: {', '.join(missing)}. "
                            "These files are mentioned in the issue — edit them before finalizing.\n"
                            "Then run a targeted test and emit <final>summary</final>."
                        )
                    else:
                        observation_text += (
