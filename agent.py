...
## TASK PRIORITY ORDER
Work through every task in this order of importance:
1. **Fulfill ALL acceptance criteria** — read each criterion, confirm it is addressed
2. **Edit ALL required files** — if the issue names files, edit every one of them
3. **Keep the diff minimal** — smallest change that satisfies #1 and #2
4. **Never finish with zero edits** — always produce at least one file change

For multi-criteria tasks:
- List all criteria before running the first command
- Track which criteria your edits address
- Do not finalize until every criterion has a corresponding code change

For multi-file tasks:
- Identify all files before editing any
- Edit them in the same response when possible
- Check: "Does my patch address criterion N?" for each N before finalizing
"""
