...
MAX_CRITERIA_NUDGES = 2     # name issue acceptance-criteria checkpoints still unaddressed (2x: catches misses that survived first nudge)
MAX_HAIL_MARY_TURNS = 2     # last-resort: force a real edit when patch is empty (2x matches Caiolaou's salvage depth)
...
def build_hail_mary_prompt(issue_text: str) -> str:
    """v26 edge: closes V04's empty-patch architectural hole. V04's
    maybe_queue_refinement opens with `if not patch.strip(): return False`,
    silently accepting empty patches as final answers. The user-shared duel
    data showed ~10-15% of V04's losing rounds at K=0% specifically for this
    failure mode.

    Empty patch -> Jaccard=0 vs any non-empty reference. Plausible-but-wrong
    edit -> Jaccard>0 with non-zero probability. Forcing a guess strictly
    dominates the empty fallback in expected value."""
...
def solve(
    ...
    def maybe_queue_refinement(assistant_text: str) -> bool:
        ...
        # v26 edge: close V04's empty-patch architectural hole. V04's original
        # `return False` silently accepted empty patches and cost ~10-15% of
        # rounds in the duel data (rounds 062453, 062464, 062432, 062451,
        # 062449 all at K=0%). HAIL MARY converts those forfeits into guesses
        # with non-zero similarity. Fires FIRST (before any other refinement)
        # because polish/syntax/test/coverage all need a non-empty patch.
        if not patch.strip():
            if hail_mary_turns_used < MAX_HAIL_MARY_TURNS:
                ...
                queue_refinement_turn(
                    ...
                    f"HAIL_MARY_QUEUED ({hail_mary_turns_used}/{MAX_HAIL_MARY_TURNS}): patch empty at refinement gate",
                )
                return True
            return False
