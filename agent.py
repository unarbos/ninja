...
def _rank_context_files(repo: Path, issue: str) -> List[str]:
    ...
    return _augment_with_test_partners(ranked, tracked_set)
...
def _check_node_syntax_one(repo: Path, relative_path: str) -> Optional[str]:
    ...
    if proc.returncode == 0:
        return None
    body = (proc.stderr or proc.stdout or "").strip().splitlines()
    msg = body[-1] if body else "node --check failed"
...
- After a focused patch and at most one cheap verification, finalize with <final>.
...
def solve(
    ...
    last_test_failure: Optional[Tuple[str, str]] = None
        ...
        for step in range(1, max_steps + 1):
                    ...
                    if patch.strip():
                        ...
                        if syntax_fix_turns_used < MAX_SYNTAX_FIX_TURNS:
                            ...
                            if errs:
                                ...
                                logs.append("\nSYNTAX_FIX_QUEUED:\n  " + "\n  ".join(errs))
                                messages.append({"role": "assistant", "content": response_text})
                                messages.append({"role": "user", "content": build_syntax_fix_prompt(errs)})
                                continue
                        if last_test_failure and test_fix_turns_used < MAX_TEST_FIX_TURNS:
                            test_path, output_tail = last_test_failure
                            test_fix_turns_used += 1
                            logs.append("\nTEST_FIX_QUEUED:\n  " + test_path)
                            messages.append({"role": "assistant", "content": response_text})
                            messages.append({"role": "user", "content": build_test_fix_prompt(test_path, output_tail)})
            ...
            for command_index, command in enumerate(command_batch, 1):
                ...
                if step >= 4 or command_index > 1:
                    ...
                    if (
                        ...
                        last_test_failure = (
                            command.strip().splitlines()[0][:160] if command else "(unknown)",
                            observation,
                        )
            ...
            if final is not None and get_patch(repo).strip():
                ...
                if junk and polish_turns_used < MAX_POLISH_TURNS:
                    ...
                    syntax_errs = _check_syntax(repo, patch_now) if syntax_fix_turns_used < MAX_SYNTAX_FIX_TURNS else []
                    if syntax_errs:
                        syntax_pending = syntax_errs
                        syntax_fix_turns_used += 1
                        logs.append("\nSYNTAX_FIX_QUEUED:\n  " + "\n  ".join(syntax_errs))
                    elif last_test_failure and test_fix_turns_used < MAX_TEST_FIX_TURNS:
                        testfix_pending = last_test_failure
                        test_fix_turns_used += 1
                        last_test_failure = None
                        logs.append("\nTEST_FIX_QUEUED:\n  " + testfix_pending[0])
            ...
            if observations:
                ...
                if polish_pending:
                    ...
                    observation_text += "\n\n" + build_syntax_fix_prompt(syntax_pending)
                elif testfix_pending:
                    observation_text += "\n\n" + build_test_fix_prompt(testfix_pending[0], testfix_pending[1])
                ...
                messages.append({"role": "user", "content": build_syntax_fix_prompt(syntax_pending)})
            elif testfix_pending:
                messages.append({"role": "user", "content": build_test_fix_prompt(testfix_pending[0], testfix_pending[1])})
            elif self_check_pending:
