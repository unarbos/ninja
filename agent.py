...
MAX_LINT_TURNS = 1         # ruff/eslint pass — keeps merged code style consistent
...
_PROJECT_HINT_FILES = (
    ...
    "setup.cfg", "tox.ini", "Makefile", "go.mod", "Cargo.toml",
...
def _project_hint_block(repo: Path, max_chars: int = 2600) -> str:
    ...
    Restored from prior king — useful so the model picks the right targeted
    test command without exploring config files."""
...
def build_preloaded_context(repo: Path, issue: str) -> str:
    ...
    # Restored: project test/build hints for verification-command selection.
...
# Restored from prior king: backtick-identifier ranking boost. Issue authors
# wrap deliberate hints in backticks; treat such identifiers as path mentions
# when they pick out a small specific set of tracked files.
...
def _rank_context_files(repo: Path, issue: str) -> List[str]:
    ...
    # Restored: backtick-wrapped identifiers in issues count as explicit
    # mentions when they pick out a small specific set of tracked files.
...
def build_lint_fix_prompt(errors: List[str]) -> str:
    ...
    return (
        "Lint issues were found in your patch. Clean these up so the merged code stays consistent:\n\n"
        f"{body}\n\n"
...
SYSTEM_PROMPT = """You are a surgical coding agent. Identify the root cause, fix it precisely and completely, and add nothing else. The best patches are minimal, well-targeted, and faithful to the codebase's existing style.
...
**Read the full issue first**: before planning, extract EVERY requirement and acceptance criterion. Issues often have multiple bullets; an incomplete patch is worse than a focused one.
...
intent for future readers — removing them when refactoring degrades the
final patch.
...
def build_self_check_prompt(patch: str, issue_text: str) -> str:
    ...
    return (
        "Self-check pass. Review your patch on three axes — correctness, completeness, "
        "and alignment with the codebase's existing style:\n\n"
        "CORRECTNESS:\n"
...
def build_hail_mary_prompt(issue_text: str) -> str:
    ...
    refinement turn. Closes a known hole at maybe_queue_refinement's early-exit
    ('if not patch.strip(): return False') which silently accepted empty
    patches. An attempt to fix the issue, even imperfectly, is more useful
    than no fix at all."""
    short = issue_text[:1500] if len(issue_text) > 1500 else issue_text
    return (
        "After all refinement attempts your patch is still empty. Take one "
        "more pass: pick the most likely target file from the preloaded "
        "snippets and make ONE plausible code edit consistent with the issue.\n\n"
        "RE-READ THE ISSUE:\n\n"
        f"{short}\n\n"
        "Pick the target file, then use sed -i, a python -c one-liner, or a "
        "heredoc to make a SINGLE TARGETED CODE CHANGE in that file. Do NOT "
        "change file modes / permissions. Do NOT add comments only. Make a "
...
# === cascade-gap detection (call sites of changed signatures) ===
...
def solve(
    ...
    state instead of propagating, so a partial fix is preserved rather than
    discarded.
    """
...
def _solve_attempt(**kwargs: Any) -> Dict[str, Any]:
    ...
    polish_turns_used = 0
    visible_surface_nudges_used = 0  # v16-B
    cascade_nudges_used = 0  # v16-B
    ...
    def maybe_queue_refinement(assistant_text: str) -> bool:
        ...
        """
        nonlocal polish_turns_used, self_check_turns_used, syntax_fix_turns_used, lint_turns_used, test_fix_turns_used, coverage_nudges_used, criteria_nudges_used, hail_mary_turns_used, total_refinement_turns_used, visible_surface_nudges_used, cascade_nudges_used, required_new_files_nudges_used, required_literals_nudges_used
        ...
        if required_new_files_nudges_used < MAX_REQUIRED_NEW_FILES_NUDGES:
            ...
            if missing_new:
                ...
                queue_refinement_turn(
                    assistant_text,
                    build_required_new_files_prompt(missing_new, issue),
        ...
        if required_literals_nudges_used < MAX_REQUIRED_LITERALS_NUDGES:
            ...
            if missing_lits:
                ...
                queue_refinement_turn(
                    assistant_text,
                    build_required_literals_prompt(missing_lits, issue),
                    "REQUIRED_LITERALS_QUEUED:\n  " + ", ".join(missing_lits),
                )
                return True

        # v16-B: visible-surface nudge (port PR#621) — UI patches must touch the rendered component
        ...
        # v16-B: cascade-gap nudge (port PR#621) — call sites of changed signatures
        if cascade_nudges_used < MAX_CASCADE_NUDGES:
            try:
                _cgaps = _cascade_gap_callers(repo, patch)
            except Exception:
                _cgaps = []
            if _cgaps:
                ...
                queue_refinement_turn(
                    assistant_text,
                    build_cascade_gap_prompt(_cgaps),
