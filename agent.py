...
MAX_STEP_RETRIES = 2
# Inner solve wall: keep below multishot outer budget / 2 so a justified retry
# has comparable time. Tau docker_solver also enforces a hard wall (>=300s)
# from exec start — see multishot constants below.
...
====================================================================
VALIDATOR DUEL / RETEST REALITY (Subnet 66)
====================================================================

You never see the king or other miners — each round is a fresh mined task. A challenger that wins the primary duel must win AGAIN on a separate retest task pool before promotion. Treat EVERY task with the same pipeline: preload-first, minimal root-cause patch, one targeted verification, then <final>. Never ship an empty patch. Round scoring blends similarity to an automated baseline with an LLM diff judge; ties favor the incumbent, so aim for decisive, issue-complete diffs with zero scope creep.
...
{repo_summary}
{context_section}
Before planning, read the ENTIRE issue above and identify every requirement (there may be more than one). Your patch must satisfy ALL of them — the LLM judge penalizes incomplete solutions. Tasks are independent draws; a primary win can be followed by a retest on different commits — never rely on lucky ordering or one-off heuristics.
...
# A 580s budget invited "retry" starts with only seconds left → killed mid-attempt
# → empty/partial patch (high retest variance). Keep outer budget under ~300s.
_MULTISHOT_TOTAL_BUDGET = 278.0
_MULTISHOT_MIN_ATTEMPT_RESERVE = 52.0
# If attempt 1 already consumed this much wall clock, skip attempt 2 — otherwise
# the process often dies before the retry finishes, which is worse than shipping
# the first (possibly thin) patch.
