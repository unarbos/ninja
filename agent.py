...
# Substrings that tau's pre-scan flags as scoring-system triggers — any
# patch line containing one of these zeros the round before the LLM judge
# even runs. The model occasionally emits these in comments or docstrings
# when it echoes back issue text, so line-level filtering inside
# get_patch() salvages the rest of the diff. king90's PR #1254 added a
# SYSTEM_PROMPT rule warning the model away from these phrases; this is
# the mechanical reinforcement (instructions can be ignored, the filter
# can't).
_TRIGGER_PHRASE_PATCH_TOKENS: Tuple[str, ...] = (
...
def get_patch(repo: Path) -> str:
    ...
    # Strip any diff line containing a tau pre-scan trigger substring. A
    # single hit zeros the round before the LLM judge runs, so line-level
    # filtering beats accepting the zero — the rest of the patch survives.
    if diff_output:
        lowered_full = diff_output.lower()
        if any(t in lowered_full for t in _TRIGGER_PHRASE_PATCH_TOKENS):
            kept_lines: List[str] = []
            for line in diff_output.splitlines():
                lowered_line = line.lower()
                if any(t in lowered_line for t in _TRIGGER_PHRASE_PATCH_TOKENS):
                    continue
...
def _extract_acceptance_criteria(issue_text: str) -> List[str]:
    ...
    requirements while stating others as imperative prose — the LLM judge
    then dings the king for "misses the core requirement" or "misses major
    requested work" on exactly those prose-only items. Destructive verbs
    (delete/remove/drop/migrate/replace/...) are included in the same
    imperative set so removal-style requirements surface as criteria too.
    Caps at _CRITERIA_MAX_BULLETS so the nudge prompt stays compact."""
    if not issue_text:
        return []
    bullets: List[str] = []
    seen_norm: set = set()
