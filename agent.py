...
# transient model error or stuck loop directly costs us rounds. Fail fast on
...
def _sanitize_output(text: str) -> str:
    ...
    Prevents malformed output from confusing the model's action parser.
    """
    if not text:
        return text
...
def format_observation(result: CommandResult) -> str:
    """Format command result for model consumption with smart output prioritization."""
    parts = [
    ...
    # Prioritize stderr if exit_code != 0, otherwise show both
...
# Strip <think>…</think> reasoning blocks some models emit before parsing
# for commands/final tags.
...
# Prevent command output containing literal <command>/<final> tags from
# confusing the model's own action parser on the next turn.
_OUTPUT_TAG_RE = re.compile(r"<(/?(?:command|final))\b", re.IGNORECASE)

# Fallback: when the model outputs markdown ```bash blocks instead of
# <command> tags, try to extract those as commands.
...
def _escape_output_tags(text: str) -> str:
    """Escape <command>/<final> tags in output to prevent parser confusion."""
...
def extract_commands(model_text: str) -> List[str]:
    ...
    # Fallback: try markdown bash code blocks
    bash_blocks = [m.group(1).strip() for m in _MARKDOWN_BASH_RE.finditer(cleaned) if m.group(1).strip()]
    if bash_blocks:
        return bash_blocks

    # Last resort: bare code blocks that look like shell commands
