...
# wrap deliberate hints (e.g. `send-expiry-emails`, `email_notificacoes`) in
# backticks; treat such identifiers as path mentions when they pick out a
# small specific set of tracked files.
...
_SYMBOL_STOP = {
    ...
    "was", "we", "with",
    # extended generic-noise tokens (merged here so _extract_issue_symbols and
    # downstream symbol-grep do not anchor on these): single authoritative list.
    ...
    "their", "there", "thing", "true", "type", "types", "update",
    "using", "value", "values", "when", "will", "without", "write",
}
