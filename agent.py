...
def _broad_grep_fallback(repo: Path, issue: str, tracked: set) -> List[str]:
    ...
    if not tracked:
        return []
    terms = [t for t in _issue_terms(issue) if len(t) >= _RESCUE_RANKER_MIN_TERM_LEN][:_RESCUE_RANKER_MAX_TERMS]
    if not terms:
        return []
    hits: Dict[str, int] = {}
    for term in terms:
