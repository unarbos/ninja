...
class _DeadlineExceeded(Exception):
    """Raised cooperatively when the multishot driver crosses the hard ceiling
    between attempts. Replaces a prior ctypes-based async-exception watchdog;
    cooperative checks are easier to reason about and have no thread-safety
    pitfalls, at the cost of only firing at attempt boundaries (which is fine
    here — _solve_attempt enforces its own internal wall-clock budget)."""


class _DeadlineGuard:
    """Cooperative deadline. Call check() at safe points (e.g. between
    multishot attempts); it raises _DeadlineExceeded once the ceiling is past.
    Also writes an on-disk patch checkpoint each time it's polled, so a
    SIGKILL still leaves a non-empty diff on disk."""

    def __init__(self, repo: Optional[Path], deadline_seconds: float) -> None:
        self._repo = repo
    ...
    def check(self) -> None:
        try:
            _write_patch_checkpoint(self._repo)
        except Exception:
            pass
        if self.remaining() <= 0:
...
def _v65_solve_with_safety_net(**kwargs: Any) -> Dict[str, Any]:
    ...
    v75: replaces the prior ctypes-based async-exception watchdog with a
    cooperative _DeadlineGuard that the multishot driver polls between
    attempts. Trade-off: cooperative checks only fire at attempt
    boundaries, so a single _solve_attempt could in principle overrun —
    but _solve_attempt has its own internal WALL_CLOCK_BUDGET (~240s)
    and the validator SIGKILL is at 600s, so the 470s ceiling here is
    only a between-attempt cap, not a mid-call interrupt. The
    _DeadlineGuard also writes an on-disk patch checkpoint each time it's
    polled, so SIGKILL still leaves a non-empty diff."""
    ...
    guard = _DeadlineGuard(_multishot_repo_obj, _HARD_DEADLINE_SECONDS)
    try:
        return _v65_multishot_driver(kwargs, _multishot_repo_obj, guard)
        ...
        return AgentResult(
            patch=salvaged or "",
            logs=f"COOPERATIVE_DEADLINE: returned at {_HARD_DEADLINE_SECONDS}s with {len(salvaged.splitlines())} lines",
...
def _v65_multishot_driver(kwargs: Dict[str, Any], _multishot_repo_obj, guard: "_DeadlineGuard") -> Dict[str, Any]:
    """The original UID195 multishot logic, separated so the safety-net try/except wraps it cleanly.
    `guard` is polled between attempts to enforce the hard ceiling cooperatively."""
    repo_path = kwargs["repo_path"]
    issue = kwargs["issue"]
    model = kwargs.get("model")
    api_base = kwargs.get("api_base")
