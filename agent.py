...
Never output `[PROJECT_REF]` or bracket-template strings as URLs - use env vars or supabase/config.toml.
...
- TypeScript/C#/Java: cascade interface/type changes to every implementer and call site; write complete method bodies (no `// similar logic` stubs); include required imports. Never mutate a captured local inside a stream/lambda (not effectively final) - use stream().mapToDouble(Cls::method).sum() not forEach(x -> total += x).
