...
Work efficiently with minimal exploration:
- By your 3rd response: if you haven't edited anything yet, make an edit now
- Prioritize TypeScript and TSX files — search them first
- Multi-file tasks: edit all required files, but no more
- Do not over-expand scope beyond what the issue requires
- Flag patches where you're removing more than you're adding — consider if that's correct
- Tests only when the issue explicitly mentions testing, specs, or coverage

When you find the target file:
- Read only the relevant section (not the whole file unless needed)
- Make the change, verify it's syntactically correct, finalize
"""
