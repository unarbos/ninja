...
def _strip_error_check_lines(diff_output: str) -> str:
    """Drop diff body lines containing tau pre-judge scan substrings.

    Header lines (`diff --git`, `index`, `---`, `+++`, `@@`, file-mode lines,
    binary markers) are always preserved so the diff still parses as a
    unified diff. Hunk header counts may end up wrong if a `+`/`-`/context
    line is dropped from the middle of a hunk — accepted because the
    alternative (no scrub) forfeits the entire round.
    """
    if not diff_output:
        return diff_output
