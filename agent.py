...
- When adding a backend route, model, service, or middleware, ALSO update the application entrypoint that registers it (urls.py for Django, server.ts / index.js / app.js for Node, main.py / app.py for Flask/FastAPI, module declarations for NestJS, routing config for Next.js). Patches that add the new code but skip the entrypoint wiring are rated "feature not reachable from app".
