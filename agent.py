...
_ERROR_LOC_RE = re.compile(
    r'(?:File "([^"]+)", line (\d+)'
    r'|^\s*at .+\(([^)]+):(\d+):\d+\)'
    r'|^([^\s:]+\.(?:py|js|ts|tsx|jsx|go|rs|java|rb|php)):(\d+))',
    re.MULTILINE,
)


def _error_hint_line(observation_text: str) -> str:
    """Return a one-line annotation when the observation contains a
    recognisable traceback or compiler error frame. Empty string on no
    match or any exception — never raises, purely additive."""
    try:
        if (
            "Traceback" not in observation_text
            and "Error" not in observation_text
            and "error" not in observation_text
        ):
            return ""
        m = _ERROR_LOC_RE.search(observation_text)
        if not m:
            return ""
        for path_grp, line_grp in ((1, 2), (3, 4), (5, 6)):
            path = m.group(path_grp)
            line = m.group(line_grp)
            if path and line:
                return f"# ERROR HINT: {path}:{line}\n"
    except Exception:
        return ""
    return ""
...
def _rank_context_files(repo: Path, issue: str) -> List[str]:
    ...
    definer_bonus = _definer_file_bonus(repo, tracked_set, issue)
    ...
    for relative_path in tracked:
        ...
        # Extra boost for files that define (not merely reference) issue symbols.
        if relative_path in definer_bonus:
            score += definer_bonus[relative_path]
        if score > 0:
...
_DEFINER_PREFIX_RE = re.compile(
    r"^\s*(?:def|class|function|export\s+(?:default\s+)?(?:function|class|const|let|var)"
    r"|const|let|var|interface|type|struct|fn|impl)\b",
)


def _definer_file_bonus(
    repo: Path,
    tracked_set: set,
    issue_text: str,
) -> Dict[str, int]:
    """Files that DEFINE issue-named symbols outrank files that merely
    reference them. Reuses _extract_issue_symbols; caps at 3 symbols and
    2 s per grep so worst-case cost is ~6 s at preload."""
    symbols = _extract_issue_symbols(issue_text)[:3]
    bonus: Dict[str, int] = {}
    for symbol in symbols:
        pattern = (
            r"^\s*(?:def|class|function|export|const|let|var|interface|type|struct|fn|impl)"
        ...
        for line in proc.stdout.splitlines():
            ...
            if not _context_file_allowed(relative_path):
                continue
            bonus[relative_path] = min(220, bonus.get(relative_path, 0) + 110)
    return bonus
...
If the issue body explicitly names a file path or directory (e.g. `path/to/foo.py` or `src/handlers/`), open and read those files with `sed -n '1,200p'` BEFORE running any `rg`/`grep` — explicit paths are ground truth, not search candidates.
...
def build_budget_pressure_prompt(step: int) -> str:
    ...
    return (
        "Final budget check: more than a third of the step budget remains but no patch exists. "
        "Emit a single `python -` heredoc that writes your best-guess fix to the most plausible file mentioned in the issue, then run the most targeted test. "
        "Returning an empty patch scores zero; a substantive imperfect patch scores partial credit."
    )
    ...
    try:
        ...
        for step in range(1, max_steps + 1):
            ...
            if observations:
                ...
                _hint = _error_hint_line(observation_text)
                if _hint:
                    observation_text = _hint + observation_text
                messages.append({"role": "user", "content": observation_text})

            if success:
                break

            if not get_patch(repo).strip() and step in {2, 4, 8, 12}:
