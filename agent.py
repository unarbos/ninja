...
def _solve_attempt(**kwargs: Any) -> Dict[str, Any]:
    ...
    def maybe_queue_refinement(assistant_text: str) -> bool:
        ...
        if polish_turns_used < MAX_POLISH_TURNS:
            # Polish only fires when the junk detector finds something concrete
            # to revert. v34's unconditional polish appeared to lift judge in
            # the 10-task bench but the 33-task bench showed the same change
            # cost rounds on tasks where the model would over-revert good edits
            # under the generic 'scan for drive-by formatting' hint. Reverting
            # to gated polish keeps the lift on junky patches without disrupting
            # clean ones.
