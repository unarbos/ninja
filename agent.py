...
After patching, run the most targeted meaningful verification available — one test case, one test file, or one module. Examples: `pytest tests/test_parser.py::test_x -q`, `pytest tests/test_x.py -x -q`, `go test ./pkg/foo`, `cargo test specific_test`, `npm test -- file -t "name"`, `mvn -q -Dtest=FooTest test`. Do not rely only on syntax checks when real targeted tests exist. Run broad suites only if the repo is small or no targeted tests exist. If no test is runnable for the changed area or dependencies block running tests, at minimum confirm each edited file still parses (`python -m py_compile f.py`, `node --check f.js`, `tsc --noEmit`, `ruby -c f.rb`, `go build ./...`) before emitting `<final>` — a patch that does not parse cannot be the reference.
