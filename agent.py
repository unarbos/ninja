...
def get_patch(repo: Path) -> str:
    ...
    filtered = _strip_low_signal_hunks(cleaned)
    if filtered.strip() and not _validate_patch_applies_cleanly(repo, filtered):
        raw = _git_raw_diff(repo)
        if raw.strip() and _validate_patch_applies_cleanly(repo, raw):
            return raw
    return filtered
...
def build_preloaded_context(repo: Path, issue: str) -> str:
    ...
    # v36 (PR596 shallowtensr): backticked path-shaped identifiers in issue that
    # match NO tracked file — likely files the task wants us to CREATE.
    ...
    # v21 edge: append recent-commit examples as concrete style anchors.
...
def _files_to_create_hint(repo: Path, issue_text: str, tracked_set: set) -> str:
    """v36 (PR596 shallowtensr): backticked path-shaped identifiers in issue
    that don't match any existing tracked file (likely files the task wants
    us to CREATE)."""
    candidates = []
    seen = set()
...
def solve(
    ...
    _combined = (_result1.get("patch") or "") + (_result2.get("patch") or "")
    if not _combined.strip():
