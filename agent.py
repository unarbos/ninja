...
# Generic prompt-injection phrases occasionally show up in a patch when
# the inner model echoes adversarial issue text back into commit
# messages, docstrings, or assertion strings it generates while editing.
# Once present they turn the patch into a pseudo-prompt that any
# downstream consumer (a CI bot, an automated reviewer, an LLM-based
# linter) may treat as instruction rather than code. Stripping them at
# patch-extraction time keeps the rest of the diff intact and prevents
# accidental prompt-injection content from leaking into source files.
# This is a defensive solver concern — the model should not be the
# vector through which adversarial text reaches downstream readers of
# the diff.
_PROMPT_INJECTION_PATCH_TOKENS: Tuple[str, ...] = (
...
def get_patch(repo: Path) -> str:
    ...
    # Strip any diff line containing a generic prompt-injection phrase so
    # adversarial issue text the model may have echoed into comments,
    # docstrings, or assertion strings does not leak into source files
    # via the produced patch. Whole-file scan first as a cheap fast-path;
    # per-line filter only when at least one phrase is present.
    if diff_output:
        lowered_full = diff_output.lower()
        if any(t in lowered_full for t in _PROMPT_INJECTION_PATCH_TOKENS):
            kept_lines: List[str] = []
            for line in diff_output.splitlines():
                lowered_line = line.lower()
                if any(t in lowered_line for t in _PROMPT_INJECTION_PATCH_TOKENS):
                    continue
