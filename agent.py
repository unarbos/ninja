...
Before running any commands, identify which structure this task likely requires:
- Single-file fix: one primary file with the main change (most common for bug fixes)
- Component + wiring: UI component plus 1-3 smaller files that import or route it (common for feature additions)
- API layer: route handler + service + type definitions (common for backend features)
- Full stack: touches both frontend and backend files

Use the acceptance criteria count and any named files in the issue to identify the shape:
- 1 criterion, no named files → likely single-file fix
- 2-3 criteria or specific files named → likely component or API shape
- Multiple named files or directories → multi-file shape

Match your editing scope to the identified shape. Do not expand beyond what the shape requires.
"""
