...
# observation size, and max_tokens.
...
# Step guardrails to force edits early and avoid timeout-heavy exploration.
FIRST_PATCH_DEADLINE_STEP = 4
PATCH_REVIEW_AUTOSTOP_STEP = 8
...
def chat_completion(
    ...
    Retries transient transport failures (timeout, connection reset, HTTP
    429/5xx) with bounded backoff. Client-side errors (4xx except 429) bail
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            if attempt < max_retries:
                ...
                continue
            raise RuntimeError(f"Model endpoint returned invalid JSON: {e}") from e
...
_BRACE_BALANCE_SUFFIXES = {
    ".c", ".cc", ".cpp", ".cs", ".go", ".h", ".hpp", ".java", ".js", ".jsx", ".kt", ".php", ".rs", ".swift", ".ts", ".tsx",
}
    ...
    while i < n:
        ...
        if ch in ")]}":
            if not stack or stack[-1] != opener_for[ch]:
                return f"{relative_path}: brace balance error near char {i + 1}"
            stack.pop()
        i += 1

    if stack:
        return f"{relative_path}: unclosed brace(s) detected"
    return None
...
Discipline:
- Scoring-critical behavior:
  1. Added-token overlap is heavily weighted, so copy local identifier names,
      literal strings, and operator patterns exactly.
  2. Hunk location overlap is heavily weighted, so verify target locations with
      focused grep/sed line reads before editing.
  3. Unrelated churn is penalized by both diff similarity and the LLM judge.
...
- If there is no patch by step 4, stop exploring and make the best direct edit
    for the highest-confidence acceptance criterion.
...
- Safety rules (explicit): no sudo, no file deletion, no network calls outside
    the validator-provided inference proxy, and no modifying hidden tests.
...
"""
    ...
    required_paths = _extract_issue_path_mentions(issue)
    required_paths_note = ""
    if required_paths:
        lines = "\n".join(f"- {path}" for path in required_paths[:12])
        required_paths_note = (
            "\nPaths explicitly mentioned in the issue (must be covered in your patch):\n"
            f"{lines}\n"
        )
...
def build_hail_mary_prompt(issue: str) -> str:
    return (
        "Hail-mary recovery: there is still no patch and the step budget is almost spent.\n\n"
        "Run one focused command that directly edits the most likely target file for this issue, "
        "then immediately provide <final>summary</final>.\n\n"
        "Issue:\n"
        f"{issue[:1000]}"
    )
...
def build_budget_pressure_prompt(step: int) -> str:
    if step < FIRST_PATCH_DEADLINE_STEP:
        return """Budget check: you have not changed the repo yet. Your next command should target exact lines with grep/sed and then edit the most likely file(s). Avoid broad exploration."""
...
def build_self_check_prompt(patch: str, issue_text: str) -> str:
    ...
    return (
        ...
        "For each hunk, justify it with one acceptance criterion. Remove any hunk that\n"
        "cannot be justified. Preserve exact local naming and literal conventions.\n\n"
        "Your patch:\n```diff\n"
        f"{truncated}\n```\n\n"
        "Task:\n"
        f"{issue_text[:2000]}\n\n"
...
def _run_companion_test_gate(repo: Path, patch: str) -> Tuple[Optional[str], str]:
    """Run one cheap companion-test check for changed Python sources."""
    ...
    for relative_path in changed:
        ...
        cmd = f"python -m pytest -q {_shell_quote(partner)}"
        result = run_command(cmd, repo, timeout=20)
        output = format_observation(result)
        lower = output.lower()
        if result.exit_code == 0:
            return None, ""
        if "no module named pytest" in lower or "command not found" in lower:
            return None, ""
...
def solve(
    ...
    def maybe_queue_refinement(assistant_text: str) -> bool:
        ...
        if test_fix_turns_used < MAX_TEST_FIX_TURNS:
            failed_test, test_output = _run_companion_test_gate(repo, patch)
            if failed_test:
                test_fix_turns_used += 1
                queue_refinement_turn(
                    assistant_text,
                    build_test_fix_prompt(failed_test, test_output),
                    f"TEST_FIX_QUEUED:\n  {failed_test}",
                )
                return True
    ...
    try:
        ...
        for step in range(1, max_steps + 1):
            ...
            for retry_index in range(1, MAX_STEP_RETRIES + 1):
                try:
                    ...
                    logs.append(f"MODEL_ERROR (attempt {retry_index}/{MAX_STEP_RETRIES}):\n{traceback.format_exc()}")
                    if retry_index < MAX_STEP_RETRIES:
                        continue
            ...
            if response_text is None:
                ...
                patch = get_patch(repo)
                if patch.strip():
                    logs.append("\nMODEL_ERROR_RECOVER:\nReturning best partial patch after persistent model errors.")
                    success = True
                    break
                if consecutive_model_errors >= MAX_CONSECUTIVE_MODEL_ERRORS:
                    logs.append("\nSTOPPED:\nToo many consecutive model-call failures with no patch.")
                    break
                messages.append({"role": "user", "content": "Model call failed. Continue with one minimal <command> edit on the highest-confidence target file."})
                continue
            ...
            for command_index, command in enumerate(command_batch, 1):
                ...
                if step >= FIRST_PATCH_DEADLINE_STEP or command_index > 1:
                    patch = get_patch(repo)
                    ...
                    if patch.strip() and result.timed_out:
                        ...
                        success = True
                        break
                    if patch.strip() and step >= PATCH_REVIEW_AUTOSTOP_STEP and _looks_like_patch_review_command(command, result):
            ...
            if not get_patch(repo).strip() and step in {2, FIRST_PATCH_DEADLINE_STEP}:
