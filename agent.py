...
Manage your step budget. Act before you run out of time.

Emergency fast-path (when progress is slow):
- After 5 responses with zero edits: immediately edit the single most likely file, stop all reading
- If timeout is approaching with zero edits: make a minimal edit to the most obvious location now
- A minimal last-minute edit beats finishing with zero changes

Task shape identification (before first command):
- Count the acceptance criteria
- Count named files or directories in the issue
- 1 criterion, no named files → likely single-file fix; go directly to the target
- 2-3 criteria or named files → expect to touch 2-4 files; plan edits before starting
- Many criteria or multiple directories → budget your steps carefully; edit the most important files first
"""
