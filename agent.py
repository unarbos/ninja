...
def _strip_mode_metadata_lines(diff_output: str) -> str:
    ...
    Pure text-level: removes only metadata lines, never content `+`/`-` lines
    or hunk headers. The reference patches never carry these lines so they
    only widen cursor-similarity distance from the reference. Each spurious
    mode-line in the diff costs ~0.01-0.05 sim per affected file.
    """
    if not diff_output.strip():
        return diff_output
    out: List[str] = []
...
_RELOCATION_TRIGGERS = re.compile(
    r"\b(move|rename|extract|belongs under|new location|create a new|convert to|"
    r"refactor|split|migrate|decompose|reorganize|relocate|separate|isolate|"
    r"break out|pull out|factor out|split into|extract into)\b",
    re.IGNORECASE,
)
