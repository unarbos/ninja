...
MAX_PRELOADED_CONTEXT_CHARS = 50000  # v1: widen preload to reduce catastrophic-floor rounds
MAX_PRELOADED_FILES = 18              # v1: more files for repos where issue spans modules
...
MAX_TOTAL_REFINEMENT_TURNS = 3  # v1: one extra refinement to recover floor on multi-file
                                # tasks where first attempt missed a required path;
                                # wider preload (50k/18) gets the first attempt closer
                                # to complete so this turn tightens rather than rewrites
                                # and stays inside the 255s wall-clock envelope
