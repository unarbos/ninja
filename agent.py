...
_ISSUE_TRACE_FILE_RE = re.compile(r'File\s+["\']([^"\']+)["\']', re.IGNORECASE)
_ISSUE_COLON_LINE_RE = re.compile(
    r"(?<![\w.-])([\w./-]+\.(?:py|ts|tsx|js|jsx|mjs|cjs|go|rs|java|kt|kts|rb|cs|cpp|cc|c|h|hpp|php|swift)):(?:\d+)(?::\d+)?",
    re.IGNORECASE,
)


def _extract_issue_trace_paths(issue: str, tracked_set: set) -> List[str]:
    """Resolve paths from tracebacks or path:line anchors in the issue text to tracked relatives."""
    found: List[str] = []
    seen: set = set()
    candidates: List[str] = []
    for m in _ISSUE_TRACE_FILE_RE.finditer(issue):
        candidates.append(m.group(1).strip())
    for m in _ISSUE_COLON_LINE_RE.finditer(issue):
        candidates.append(m.group(1).strip())
    ...
    for raw in candidates:
        ...
        if raw in tracked_set and _context_file_allowed(raw):
            ...
            for start in range(len(parts)):
                ...
                if suffix in tracked_set and _context_file_allowed(suffix):
                    resolved = suffix
                    break
            if not resolved and len(raw) >= 10:
                for rel in tracked_set:
                    if _context_file_allowed(rel) and (rel.endswith(raw) or raw.endswith(rel)):
                        resolved = rel
                        break
...
def _rank_context_files(repo: Path, issue: str) -> List[str]:
    ...
    trace_hit_set = frozenset(_extract_issue_trace_paths(issue, tracked_set))
    for tp in trace_hit_set:
        if tp not in seen_mentioned:
            mentioned.append(tp)
            seen_mentioned.add(tp)
    ...
    for relative_path in tracked:
        ...
        if relative_path in trace_hit_set:
            score += TRACE_PATH_SCORE_BOOST
        if score > 0:
