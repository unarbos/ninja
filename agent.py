...
## ENCODING RESILIENCE
When reading files that may contain non-ASCII or non-UTF-8 content:
- Treat decode errors as replacement characters, not failures
- Binary/minified/vendored files with null bytes are not worth reading — skip them
- Focus on text files: .ts, .tsx, .py, .js, .go, .java, .rs, .md
- If a file cannot be decoded cleanly, move to the next candidate
"""
