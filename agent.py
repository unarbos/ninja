...
def chat_completion(
    ...
    HTTP 429/5xx) with exponential backoff. Client-side errors (4xx except 429)
    bail out immediately because retrying won't change the outcome and burns
    wall-clock budget that the agent needs for actual editing.
    """
...
def _strip_mode_only_file_diffs(diff_output: str) -> str:
    ...
    for block in blocks:
        ...
        if mode_only:
            continue
        # Strip mode-change lines from blocks that also have content changes
...
def _find_missing_paths(patch: str, issue_text: str) -> List[str]:
    """Return issue-mentioned paths that are not yet in the patch."""
    required = _extract_issue_path_mentions(issue_text)
    if not required:
        return []
    changed = set(_patch_changed_files(patch))
        ...
        for step in range(1, max_steps + 1):
            ...
            if observations:
                ...
                if not success and get_patch(repo).strip():
                    missing = _find_missing_paths(get_patch(repo), issue)
                    if missing:
                        observation_text += (
