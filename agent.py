...
MAX_TOTAL_REFINEMENT_TURNS = 3  # coverage + correctness + cleanup, still bounded
...
def _scope_gap_summary(patch: str, issue_text: str) -> str:
    ...
    if any(term in lower for term in ("ui", "page", "screen", "component", "dashboard", "admin", "form", "modal")):
        if not any(term in changed_text for term in ("page", "screen", "component", "view", "template", "layout", "admin", "dashboard", ".vue", ".tsx", ".jsx")):
    ...
    if any(term in lower for term in data_owner_terms):
        api_hit = any(term in changed_text for term in ("api", "route", "controller", "service", "repo", "model", "schema", "migration", "entity"))
        ui_requested = any(term in lower for term in ("ui", "page", "screen", "form", "dashboard", "admin"))
        ui_hit = any(term in changed_text for term in ("page", "component", "view", ".vue", ".tsx", ".jsx", "template"))
        if not api_hit:
            closure_gaps.append("server/data owner not touched")
        if ui_requested and not ui_hit:
...
- Keyboard/editor shortcuts: handle explicit modifier shortcuts only (Ctrl/Meta/Alt as appropriate). Do not hijack plain typing such as `c`, `x`, or `v`; preserve normal text input and re-run the editor's existing parse/render update after paste.
- Class extraction/refactor: preserve public fields and methods expected by callers, even on lightweight subclasses. If the original type exposed ID/name/status-like fields, keep compatible fields or adapters unless the issue explicitly removes them.
...
def build_budget_pressure_prompt(step: int) -> str:
    if step < 4:
        return (
            ...
            "Do not run another pure inspection command. A precise sed or python -c is better than another grep. Stop exploring."
        )
    return (
...
def build_scope_nudge_prompt(scope_summary: str, issue_text: str) -> str:
    return (
        ...
        f"  {scope_summary}\n\n"
        "Do not finalize yet. First inspect the missing owner with one focused `rg` or `sed -n`: "
...
def _solve_attempt(**kwargs: Any) -> Dict[str, Any]:
    ...
    def maybe_queue_refinement(assistant_text: str) -> bool:
        ...
        """
        nonlocal polish_turns_used, self_check_turns_used, syntax_fix_turns_used, static_fix_turns_used, test_fix_turns_used, coverage_nudges_used, criteria_nudges_used, scope_nudges_used, hail_mary_turns_used, total_refinement_turns_used
        ...
        # If the attempt is near the wall-clock reserve, avoid broad
        # completion nudges. A small static/syntax fix is still worthwhile; a
        # new feature-expansion turn is more likely to time out than to help.
        allow_broad_refinement = time_remaining() > 70.0
        ...
        if allow_broad_refinement and static_fix_turns_used < 1:
