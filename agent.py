...
MAX_STEP_RETRIES = 2
# Inner wall kept below outer multishot budget so retry has comparable time.
...
def build_preloaded_context(repo: Path, issue: str) -> Tuple[str, List[str]]:
    ...
    # Rescue-ranker: when top_score is weak (no path mention, no symbol-grep
    # hit), the ranker output is effectively random. Fall back to broad-grep
    # over the full tracked set and prepend the best matches plus a banner.
    ...
    if rescue_files:
        # Banner before any file snippet so the model reads it first.
        rescue_banner = (
            "### rescue-ranker hint\n"
...
# Outer budget kept under tau docker_solver's ~300s hard wall (was 580s).
_MULTISHOT_TOTAL_BUDGET = 278.0
_MULTISHOT_MIN_ATTEMPT_RESERVE = 52.0
# Skip retry if attempt 1 burned the outer budget — better to ship a thin
# patch than be killed mid-retry.
...
def _solve_with_safety_net(**kwargs: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if _elapsed > _MULTISHOT_MAX_FIRST_ELAPSED:
            # Retry would risk a docker_solver kill mid-attempt.
            _result1["multishot_attempts"] = 1
