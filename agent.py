...
_MINIFIED_AVG_LINE_THRESHOLD = 200   # avg added-line length above this = minified
_MINIFIED_MIN_LINES_TO_CHECK = 5     # don't classify tiny diffs as minified


def _strip_minified_content_diffs(diff_output: str) -> str:
    """Strip diff blocks whose CONTENT looks minified, even when the path
    didn't match a `*.min.js` / `*.bundle.*` pattern.

    Duel evidence (round vs new king on react-pdf-1280): without this filter,
    the candidate emitted a 1.3 MB patch consisting almost entirely of a
    minified UMD bundle file. The LLM judge gave that round score 0 ("edits
    built artifacts only and includes scope-creep noise"). Stripping the
    minified-content block before it reaches the judge prevents the model
    from accidentally torpedoing its own score on JS/TS repos that ship
    pre-built bundles alongside source.

    Detection heuristic: for each diff block, compute the average length of
    added/context lines. Minified JS/CSS typically has avg line length >200
    chars (the whole file is often one line per statement after compression).
    Real source code averages 40-100 chars per line.

    Conservative defaults: needs at least _MINIFIED_MIN_LINES_TO_CHECK lines
    of content to classify (small diffs aren't dropped); only strips blocks
    where the AVG (not max) exceeds the threshold, so legitimate source files
    with one or two long lines aren't impacted.
    """
    if not diff_output.strip():
        return diff_output
...
def _should_skip_patch_path(relative_path: str) -> bool:
    ...
    if path.suffix in {".pyc", ".pyo"}:
        return True
    # Lockfiles regenerate on every install — they hitchhike into patches as
    # scope-creep noise the LLM judge penalises.
    if path.name in {
        "package-lock.json",
        "yarn.lock",
        "pnpm-lock.yaml",
        ...
        return True
    # Minified / bundled build outputs. Models often wander into these via
    # grep on JS/TS repos that ship pre-built UMD/ESM artefacts alongside
    # source. The _strip_minified_content_diffs filter catches them at the
    # diff-content level, but skipping the path earlier is cheaper and more
    # robust when the bundle is touched by an untracked-file path.
    name_lower = path.name.lower()
    minified_suffixes = (
        ".min.js", ".min.css", ".min.mjs",
        ".bundle.js", ".bundle.css", ".bundle.mjs",
        ".umd.js", ".umd.cjs", ".umd.min.js",
        ".esm.js", ".esm.cjs",
        ".compiled.js", ".compiled.css",
    )
    if any(name_lower.endswith(suffix) for suffix in minified_suffixes):
        return True
    generated_parts = {
        "__pycache__",
        ".pytest_cache",
...
def _run_companion_test(
    ...
    if suffix in {".ts", ".tsx", ".js", ".jsx", ".cjs", ".mjs"}:
        # Try real test runner first if locally installed. JS/TS tests are
        # the dominant correctness signal on this task pool (chartjs, marked,
        # react-pdf, p5.js all ship with jest configs and node_modules/.bin/jest
        # populated). King's existing implementation only does `node --check`
        # (syntax-only), missing real test failures.
        js_runner_cmd = _detect_js_test_command(repo, test_path)
        if js_runner_cmd is not None:
            js_timeout = max(timeout_seconds, 15)
            try:
                proc = subprocess.run(
                    js_runner_cmd,
                    cwd=str(repo),
                ...
                # Tests genuinely slow → fall back to node --check below.
                proc = None
            except Exception:
                proc = None
            if proc is not None:
                ...
                if not any(marker in output for marker in unrunnable_markers):
                    if proc.returncode == 0:
                        return None  # test passed
                    return output[-2400:] if len(output) > 2400 else output
            # Runner unrunnable — fall through to syntax check.
...
def _detect_js_test_command(repo: Path, test_path: str) -> Optional[List[str]]:
    """Return runnable test command for a JS/TS file, or None.

    Strategy: prefer locally-installed test runner binaries (node_modules/.bin/
    jest, vitest, mocha) because they need no global install and use the
    repo's exact resolved config. If no .bin entry exists, return None and
    let the caller fall back to `node --check`.

    Why this is a structural lift over king: the king's _run_companion_test
    explicitly skipped JS/TS test runners ("they require project-level config
    we can't synthesize"). But on real validator clones of JS-heavy repos
    (chartjs, marked, react-pdf, vuepress, etc), node_modules is typically
    already populated — we just need to invoke the bin directly.
    """
    bin_dir = repo / "node_modules" / ".bin"
    if not bin_dir.is_dir():
        return None
    # Probe in popularity order: jest > vitest > mocha
    candidates: List[Tuple[str, List[str]]] = [
        ("jest", ["jest", "--no-coverage", "--silent", "--colors=false",
                  "--testPathPattern", test_path]),
        ("vitest", ["vitest", "run", "--no-color", test_path]),
        ("mocha", ["mocha", "--no-colors", "--reporter", "min", test_path]),
    ]
    for binary, argv in candidates:
        bin_path = bin_dir / binary
        if not bin_path.exists():
            continue
        return [str(bin_path)] + argv[1:]
    return None
...
def _is_test_path(relative_path: str) -> bool:
    ...
    parts = lower.split("/")
    if {"test", "tests", "__tests__", "spec"} & set(parts):
        return True
...
def _select_companion_test_failure(
    ...
    """For files touched by the patch, find the first failing test.

    Discovery order (king only used #2):
      1. Test paths mentioned literally in the issue text (highest precision —
         issue often quotes the failing test path).
      2. Companion tests for each edited source file (king's existing approach).
    ...
    # 1. Issue-mentioned test paths — issues frequently quote the failing test
    # file explicitly (e.g. "tests/test_parser.py::test_x fails"). King's path-
    # mention extraction already exists; just filter to test-shaped paths.
    candidates: List[str] = []
    seen: set = set()
    if issue_text:
        try:
    ...
    # 2. Companion tests for edited files (king's existing logic).
...
# Error-string file boost: a file-selection signal not exploited by any prior
# king PR (verified up through PR #1576). Past winners boosted files via path
# mentions, backtick idents, snake/Camel idents, symbol grep, integration
# partners, sibling dirs, and the recently-tightened identifier stopword set —
# but never via the exact quoted English phrases (error messages, expected
# output) that issues routinely include verbatim.
_QUOTED_STRING_RE = re.compile(r"`([^`\n]+)`|\"([^\"\n]+)\"|'([^'\n]+)'")
_ERROR_STRING_MIN_LEN = 20
_ERROR_STRING_MAX_LEN = 200
_ERROR_STRING_MAX_PATTERNS = 5
_ERROR_STRING_MAX_FILES_PER_PATTERN = 10  # >10 hits = too generic
...
def build_triage_prompt(issue_text: str, preloaded_context: str) -> str:
    """Pre-solve structured planning prompt. Single focused call that shapes
    the main solve's direction before the autonomous loop starts. Output is
    5 short lines; the main solve sees this prepended as a HINT (not directive).

    Untried by any prior king (verified through #1576): all kings rely on the
    model's free-form <plan> step in turn 1. A separate, constrained planning
    call has a different cognitive frame ("you're a triage assistant focused
    on structure") and produces more disciplined output that primes the main
    solve with concrete file/function targets before the autonomous agent
    spends steps re-discovering them.
    """
    truncated_ctx = preloaded_context[:1500] if preloaded_context else ""
    return (
        "You are a focused bug-triage assistant. Read the issue + context and "
        "produce EXACTLY 5 lines, no preamble, no markdown, no extra prose. "
        "Each line follows the exact format shown. If unsure on a line, write "
        "'unknown' — do not make up paths or functions.\n\n"
        f"ISSUE:\n{issue_text[:1800]}\n\n"
        f"PRELOADED CONTEXT (top-ranked file snippets):\n{truncated_ctx}\n\n"
        "Output exactly these 5 lines:\n"
        "TYPE: <crash | logic-error | missing-feature | refactor | test-fix | "
        "api-change | doc | other>\n"
        "TARGET_FILE: <single most-likely file path to edit, or 'unknown'>\n"
        "TARGET_FUNCTION: <function/class name to edit, or 'unknown'>\n"
        "FIX_APPROACH: <one sentence — what the minimal surgical change should be>\n"
        "EDGE_CASE: <one edge case the fix must handle, or 'none'>\n"
    )


def _run_pre_solve_triage(
    issue_text: str,
    ...
    """Run a fast pre-solve triage call. Returns a formatted triage block to
    prepend to the main user message, or empty string on any failure/timeout.

    Fails open: triage is best-effort. If the API call fails, times out, or
    returns garbage, the main solve runs without it (same as king's behavior).
    """
    if not issue_text or len(issue_text) < 40:
        return ""
    prompt = build_triage_prompt(issue_text, preloaded_context)
    messages = [
        {
            "role": "system",
            "content": (
                "You are a focused bug-triage assistant. Output exactly 5 "
                "structured lines, no more, no less. No code blocks."
            ),
        },
    ...
    # Lightweight format check — at least the first two keys must appear.
    if "TYPE:" not in text_clean or "TARGET_FILE:" not in text_clean:
        return ""
    return (
        "PRE-SOLVE TRIAGE (one focused planner-call's read on this bug — treat "
        "as a hint, not absolute truth; verify against the issue and preloaded "
...
def _solve_attempt(**kwargs: Any) -> Dict[str, Any]:
    ...
    def maybe_queue_refinement(assistant_text: str) -> bool:
        ...
        if test_fix_turns_used < MAX_TEST_FIX_TURNS:
            failure = _select_companion_test_failure(repo, patch, issue_text=issue)
    ...
    try:
        ...
        # Pre-solve triage: one focused planning call that primes the main
        # solve with concrete file/function targets and fix-approach. Fails
        # open (empty string on any error). Adds ~5-15s wall-clock.
        _triage_block = _run_pre_solve_triage(
            issue, preloaded_context, model_name, api_base, api_key,
        )
