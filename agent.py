...
# Criteria-starved retry: when attempt 1 produces a substantial diff but still
# leaves this many extracted issue requirements unaddressed, force a retry.
# Catches the "large but incomplete" failure mode that the line-count-only
# threshold misses — patches that emit thousands of lines but skip several
# numbered acceptance bullets. Set to 4 (calibrated against typical issue
# bullet counts of 5-8 — half-or-more unaddressed triggers retry).
_MULTISHOT_INCOMPLETE_CRITERIA_THRESHOLD = 4
...
def _solve_with_safety_net(**kwargs: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        # Criteria-starved retry: even when attempt 1 has substantive line
        # count, force a retry when it leaves many extracted acceptance-criteria
        # bullets unaddressed. Catches "large but incomplete" patches that the
        # line-count threshold alone passes through. The criteria signal uses
        # the same _unaddressed_criteria heuristic the refinement gate uses;
        # threshold-misses here are the dominant 34% incomplete loss reason.
        _issue = kwargs.get("issue", "") or ""
        _unaddressed1 = (
            _unaddressed_criteria(_patch1, _issue) if _patch1.strip() else []
        )
        _criteria_starved = (
            len(_unaddressed1) >= _MULTISHOT_INCOMPLETE_CRITERIA_THRESHOLD
        )

        if _n1 >= _MULTISHOT_LOW_SIGNAL_THRESHOLD and not _criteria_starved:
