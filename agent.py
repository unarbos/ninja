...
# Footprint thresholds — STRICT profile. Tighter caps + lower trigger so the
# shrink turn fires on medium-broad patches that core would not flag. Targets
# the "mostly correct but too large" failure mode against gpt-5.4's anti-churn
# bias. Same scoring code, different constants — partial decorrelation.
_V14_FOOTPRINT_TOTAL_LINES_CAP = 400  # core: 600
_V14_FOOTPRINT_FILE_BUDGET_BASE = 3   # core: 4
_V14_FOOTPRINT_FAT_HUNK_LINES = 60    # core: 80
_V14_FOOTPRINT_HUGE_HUNK_LINES = 120  # core: 160
_V14_FOOTPRINT_MEDIUM_TOTAL = 150     # core: 220
_V14_FOOTPRINT_LARGE_TOTAL = 250      # core: 350
_V14_FOOTPRINT_LARGE_FAT = 90         # core: 120
_V14_FOOTPRINT_TRIGGER_PENALTY = 14   # core: 18  — fires earlier on medium-bad
...
def _solve_attempt(**kwargs: Any) -> Dict[str, Any]:
    ...
    def maybe_queue_refinement(assistant_text: str) -> bool:
        ...
        # V14 — continuous footprint penalty. Replaces v3b's blunt
        # "binary >600 lines" trigger with a score that fires on the medium-
        # bad patches the binary check missed. Runs after coverage/criteria
        # so we already have the right files; runs before self-check so the
        # model still has budget to actually shrink.
        if footprint_turns_used < MAX_FOOTPRINT_TURNS:
            try:
                fp = _v14_patch_footprint(patch)
                penalty = _v14_footprint_penalty(fp, issue)
            except Exception:
                fp, penalty = None, 0
            if fp is not None and penalty >= _V14_FOOTPRINT_TRIGGER_PENALTY:
                footprint_turns_used += 1
                total_refinement_turns_used += 1
                queue_refinement_turn(
                    assistant_text,
                    _build_v14_footprint_shrink_prompt(fp, penalty, issue),
                    f"V14_FOOTPRINT_SHRINK_QUEUED:\n  files={fp['files']} "
                    f"changed={fp['total_changed']} largest={fp['largest_hunk']} "
                    f"fat={fp['fat_hunks']} penalty={penalty}",
                )
                return True
    ...
    try:
        ...
        # V14 — light excerpt augmentation around issue symbols in top file.
        try:
            _v14_excerpt_block = _v14_build_excerpt_block(repo, issue)
            if _v14_excerpt_block:
                preloaded_context = preloaded_context + "\n\n" + _v14_excerpt_block
        except Exception:
            pass

        # V14 — profile-aware system prompt. surgical = minimum-change; the
