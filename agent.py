...
def build_preloaded_context(repo: Path, issue: str) -> str:
    ...
    check_preloaded_files = MAX_PRELOADED_FILES * 2
    for relative_path in files[:check_preloaded_files]:
...
- Read the files that need to change IN FULL before editing.
- Identify the MINIMAL set of changes — every extra line hurts your score.
- Make precise, targeted edits. Match existing code style exactly.
- Stop. Do not summarize, verify, or re-read files.
        ...
        for step in range(1, step_count + 1):
            ...
            if not commands:
                ...
                if patch.strip():
                    ...
                    success = True
                    break
                no_command_counter = MAX_NO_COMMAND_REPAIRS * 2
                if consecutive_no_command >= no_command_counter:
