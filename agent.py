...
def _solve_attempt(**kwargs: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        # Disable executable-bit tracking so incidental container/umask/sed
        # mode drifts do not emit `old mode`/`new mode` lines in the diff.
        # The reference patches never carry mode lines; any such metadata
        # widens cursor-similarity distance. Repo-local config only.
        try:
            subprocess.run(
